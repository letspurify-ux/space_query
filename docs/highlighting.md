...
| --- | --- | --- | --- |
| relation 이름 문맥 | `FromClause`, `IntoClause`, `UpdateTarget`, `DeleteTarget`, `MergeTarget` | `FROM`, `INSERT INTO`, `UPDATE target`, `DELETE target`, `MERGE INTO` | 테이블/뷰를 우선 추천한다. visible CTE 이름도 함께 합쳐진다. 빈 prefix와 non-empty prefix 모두 relation/CTE 계열만 유지하고 일반 keyword/function 후보로 넓히지 않는다. |
