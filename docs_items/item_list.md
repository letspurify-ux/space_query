...
| 6 | 11. 읽기 전용 커넥션 | `sql_classification.rs` 재사용, 안전성 |
| 7 | 3. CSV 임포트 | 비용은 크지만 "구멍"이 가장 명확한 항목 |
| 8 | 5. Drop/Truncate (미리보기 후 실행) | 범위를 좁히면 작음 |
| 9 | 16. 미저장 탭 복원 | 데이터 손실 방지 |
| 10 | 6. 컬럼 숨김/순서 | 구조 변경 수반 |
...
| 헤더 정렬 리다이렉트 | `ResultTableWidget::try_redirect_header_sort` |
...
| 4 | 헤더 정렬 리다이렉트 | 필터 활성/비활성 각각의 클릭 동작 테스트 |
...
부록 A.3(3)과 같은 작업이라 그쪽에서 함께 처리한다.
