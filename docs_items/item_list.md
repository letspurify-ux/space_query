...
작성일: 2026-08-06 · 갱신일: 2026-08-08 (**3차 재조사** — 항목 34~41 추가,
완료 항목의 남은 조각을 한 표로 모음. 그 전 갱신: 항목 6·16·24·25·26·27 완료 기록,
항목 4·11 완료 기록, 항목 28 재검증 기록, 항목 31 완료 기록, 항목 10·12·28 완료 기록,
항목 7·8 완료 기록, 구현 상태 재확인, 항목 24~33 추가)
...
- **제외 추가 (2026-08-08 3차 재조사)**: 클립보드 → 그리드 붙여넣기(TSV/여러 셀·
  단일 값의 선택 영역 채우기까지, `result_table.rs:4617`
  `paste_clipboard_text_into_edit_mode`), 객체 트리 `Copy Name`
  (`object_browser.rs:6743`), 쿼리 히스토리의 텍스트 검색 + 실패만 보기
  (`query_history.rs:553`), 탭을 닫을 때의 `Commit and Close` 선택
  (`main_window.rs:6302`). 넷 다 DataGrip 대비 구멍으로 적을 뻔했으나 이미 있다.
...
옳을 수 없다. → Transpose는 [38번](#38-단일-레코드transpose-뷰), BLOB은
[35번](#35-셀-값--파일-lob을-파일로-저장--파일을-셀에-적재)으로 옮겼다.
35번은 "화면에서 편집"이 아니라 **파일로 주고받는** 형태라 위 사유와 충돌하지 않는다.
...
→ 추적을 위해 [37번](#37-객체-생성구조-변경-ddl-create--modify-table--rename--comment)으로
옮겼다(코멘트 편집도 같은 성격이라 함께 묶었다).
...
#### 구현 상태 (2026-08-08) — 완료 (숨김 · 순서 변경)

그리드 컨텍스트 메뉴 **Columns...** 가 컬럼별 표시 체크 + Move Up/Down + Reset
모달을 연다.

| 조각 | 위치 |
| --- | --- |
| 배치 규칙·치환 (순수 함수, 테스트 13개) | `src/ui/column_layout.rs` |
| 모달 | `src/ui/column_layout_dialog.rs` |
| 적용 | `ResultTableWidget::apply_column_layout` / `column_layout_plan` |
| 진입점 | `ResultTabsWidget::arrange_visible_result_columns` |
| 라이브 검증 (4 백엔드) | `src/bin/verify_grid_features_live.rs` |
| 캡쳐 | `capture_feature_tour column-layout` |

설계상 결정 네 가지:

- **표시 매핑이 아니라 물리적 치환이다.** 난이도 항목이 예상한 "표시 인덱스 ↔
  데이터 인덱스 매핑"은 채택하지 않았다. draw 콜백·모든 히트 테스트·키보드 이동
  헬퍼 15개·편집 세션·검색·집계가 전부 **컬럼 인덱스 하나**로 돌아가고 있어서,
  매핑을 넣으면 그 전부를 좌표 변환해야 한다. 대신 `headers`·`full_data`·
  `column_kinds`·`column_data_types`·폭을 실제로 치환하면 불변식이 유지되므로
  **하류가 한 줄도 바뀌지 않는다.**
- **숨김은 폭 0 집합이다.** 기존에 기술 컬럼(ROWID) 하나를 숨기던 방식을 집합으로
  일반화했다 — `Option<usize>`가 `HiddenColumns`가 되면서 컴파일러가 28개 시그니처와
  159개 참조를 전부 짚어 줬다. `grid_search`·`selection_summary`도 같은 타입을 받는다.
- **기술 컬럼은 잠긴다.** ROWID 열은 목록에 뜨지도 움직이지도 않고 자기 인덱스를
  지킨다. 그래야 편집 서술자의 `snapshot_column_index`가 유효한 채로 남는다.
- **편집과 배타적이다.** 편집 세션은 행마다 컬럼 인덱스를 짝지어 들고 있으므로,
  Edit이 켜져 있거나 저장이 진행 중이면 배치 변경을 거절하고 사유를 말한다.
  반대 방향도 막는다.

**남은 것**: 고정(pin). FLTK `Table`에 프리즈 개념이 없어 커스텀 드로잉이 필요하고,
이번 요청 범위에 없었다.
...
#### 구현 상태 (2026-08-08) — 완료 (하이브리드 = (a) + (b))

`grid_sort.rs`를 커밋 `f635179^`에서 그대로 되살리고(611줄, 순수 함수, 테스트 27개)
결과 탭마다 **둘 중 정확히 하나만** 살아 있게 하는 배타 게이트를 붙였다.

| 결과 | 정렬 경로 |
| --- | --- |
| 필터 바가 붙음 = 재조회 가능 | 헤더 트리거 → 필터 바 `ORDER BY`를 채우고 서버 재조회 |
| 필터 바 없음 | `grid_sort` 로컬 정렬 + 헤더 정렬 표시자 |

| 조각 | 위치 |
| --- | --- |
| 비교 로직 (순수 함수, 테스트 27개) | `src/ui/grid_sort.rs` (복구) |
| 리다이렉트 설치 | `ResultTabsWidget::build_filter_bar` |
| NULL 위치 주입 | `set_sort_null_ordering_by_id` ← `DatabaseType::sorts_nulls_last_ascending` |
| 라이브 검증 (4 백엔드) | `src/bin/verify_grid_features_live.rs` |
| 캡쳐 | `capture_feature_tour grid-sort` |

설계상 결정 세 가지:

- **리다이렉트를 필터 바를 만드는 자리로 옮겼다.** 제거 이전에는
  `attach_result_filter_bar_by_id`에만 설치돼 있어서 **테이블 브라우징 탭은
  로컬 정렬로 떨어졌다** — 페이지 단위 정렬, 즉 부록 B.2의 결함 5다. 이제
  `build_filter_bar` 한 곳에서 설치하므로 필터 바가 있는 탭은 예외 없이 서버로 간다.
  결함 5가 이번에 실제로 닫혔다.
- **트리거는 원래 것(헤더 더블클릭) 그대로.** 좌클릭은 이미 폭 조절·드래그 선택과
  제스처를 공유하므로 새 판정을 만들지 않았다.
- **collation은 흉내내지 않는다**(부록 B.3 (c)). 라이브 검증은 숫자 컬럼과 NULL
  위치를 서버 `ORDER BY`와 대조한다 — Oracle NULLS LAST / MySQL 계열 FIRST가
  4백엔드에서 실제로 일치했다.
...
#### 구현 상태 (2026-08-08) — 완료 (설계 변경: 필터 바 주입 → 로컬 필터)

**위 난이도 항목의 설계는 채택되지 않았다.** 사용자 결정으로 두 필터가 **배타적**이
됐다: 필터 바가 있으면 셀값 빠른 필터를 아예 띄우지 않고, 필터 바가 없는 결과
(= 재조회 불가능한 결과)에서만 띄운다. 그 결과 넣을 `WHERE` 칸이 존재하지 않으므로
SQL을 만드는 대신 **이미 가져온 행을 로컬로 거른다.**

| 조각 | 위치 |
| --- | --- |
| 매칭·설명 (순수 함수, 테스트 14개) | `src/ui/grid_value_filter.rs` |
| 상태 표시줄 위젯 | 같은 파일의 `GridValueFilterBar` |
| 적용/해제 | `ResultTableWidget::apply_value_filter_after_fetch_all` / `clear_value_filter` |
| 진입점 | `ResultTabsWidget::apply_visible_value_filter` |
| 라이브 검증 (4 백엔드) | `src/bin/verify_grid_features_live.rs` |
| 캡쳐 | `capture_feature_tour value-filter` |

설계상 결정 네 가지:

- **평가기를 만들지 않았다.** 1번이 로컬 평가기를 기각한 이유(3값 논리·암묵 형변환·
  collation·함수 지원 범위)는 여기 하나도 해당하지 않는다. 파싱도 형변환도 없고,
  사용자가 셀을 가리키면 그 컬럼의 텍스트가 바이트 단위로 같은 행만 남는다.
  NULL 판정은 그리드가 이미 쓰는 규칙 그대로다.
- **제외는 정확한 여집합이다.** `NOT (...)`로 감싸면 NULL이 든 행이 UNKNOWN이 되어
  **양쪽 어디에도 안 걸린다.** 그래서 원자마다 여집합을 만들고 De Morgan으로 합친다.
  두 방향이 모든 행을 정확히 이분한다는 것을 유닛 테스트와 라이브 검증 양쪽에서 고정했다.
- **편집과 배타적이다.** 필터는 행을 숨기고 편집 세션은 화면의 행에 상태를 짝지으므로
  양방향으로 막는다.
- **부분 fetch 상태에서는 fetch를 먼저 끝낸다.** 단 필터 자체는 **즉시** 만든다 —
  사용자가 가리킨 셀이 아직 화면의 그 셀일 때 값을 확정해야 한다.

**캡쳐 검증이 실제 결함 하나를 잡았다.** `queue_action_after_fetch_all`이 액션을
값으로 가져가고 lazy fetch가 없으면 그대로 버리는데, 큐에 먼저 넘긴 뒤 되찾으려 해서
**lazy fetch가 없는 흔한 경우에 필터가 조용히 아무 일도 하지 않았다.** 유닛 테스트는
순수 모듈만 봤기에 못 잡았고, 캡쳐 스크린샷의 그리드가 안 걸러진 것으로 드러났다.
이제 세션 유무를 **넘기기 전에** 확인한다.

**남은 것**: 필터는 값 일치만 본다. 범위나 부분 문자열 조건은 없다
(부분 문자열은 `Ctrl+F` 그리드 검색이 이미 한다).
...
테이블 컨텍스트 메뉴 **Export Data...** 가 결과 그리드와 **같은 모달**을 띄우고
**같은 바이트**를 낸다.

| 조각 | 위치 |
| --- | --- |
| 포맷 분기 (그리드와 공유) | `result_export::render_export_content` / `with_destination_prelude` |
| 조회 (백엔드별 seam) | `ObjectBrowserDbBehavior::load_table_rows` / `export_select_sql` |
| thin용 QueryResult 실행기 | `ObjectBrowser::execute_thin_query` |
| 렌더 | `ObjectBrowserWidget::render_table_export` |
| 전달 | `SqlAction::ExportData` → `MainWindow::deliver_object_export` |
| 라이브 검증 (4 백엔드 × 7 포맷) | `src/bin/verify_grid_features_live.rs` |

설계상 결정 네 가지:

- **포맷 분기를 그리드에서 들어냈다.** `ResultTableWidget::render_export`에 갇혀
  있던 "SqlInserts만 dialect 경로" 판정을 `result_export`의 자유 함수로 옮겨
  **분기가 한 곳**이 됐다. 두 경로가 포맷 라우팅을 두고 갈릴 수 없다.
- **thin에 QueryResult 실행기가 없어서 만들었다.** 그러면서 편집기에 있던
  `oracle_thin_column_info`(컬럼 타입 → `SqlValueKind`)를 `executor.rs`로 옮겨
  **매핑을 하나로** 만들었다 — 두 벌이면 같은 테이블이 트리에서와 그리드에서
  다르게 분류될 수 있었다.
- **읽기 전용에서도 뜬다.** `WRITE_CAPABLE_MENU_LABELS`에 넣지 않았다.
- **쓰기 경로는 하나다.** 파일 선택·쓰기·상태 보고를 `MainWindow`의 기존
  `FileActionResult` 경로에 그대로 맡긴다.

**가드가 결함 하나를 잡았다.** MySQL 조회를 `MysqlExecutor::execute`로 썼는데
이건 MySQL로 기본값을 먹는 API라 **MariaDB가 MySQL 취급**을 받았다.
`db_dispatch_guards`가 잡아서 `execute_for_db_type`으로 고쳤다.

**남은 것**: 진행률 표시와 취소. 조회는 워커 스레드라 UI가 멈추지는 않지만,
큰 테이블은 끝날 때까지 되돌릴 수 없다.
...
#### 구현 상태 (2026-08-08) — 완료 (컬럼만)

테이블 노드에서 `→` 또는 확장 화살표를 누르면 컬럼이 `NAME  TYPE` 형태의 자식으로
붙는다.

| 조각 | 위치 |
| --- | --- |
| 캐시 | `ObjectCache::table_columns` |
| 라벨·역조회 | `column_node_label` / `column_name_for_node_label` |
| 지연 로딩 | `table_requiring_column_load` → `load_table_columns_async` → `ObjectActionResult::TableColumns` |
| 노드 해석 | `ObjectItem::Column` + `column_item_info` |
| 캡쳐 | `capture_feature_tour tree-columns` |

설계상 결정 네 가지:

- **패키지 선례를 그대로 따랐다.** 트리는 필터 한 글자마다 `ObjectCache`에서 통째로
  다시 만들어지므로, 위젯에 직접 붙인 노드는 다음 타이핑에 사라진다. 컬럼을 캐시에
  넣고 `collect_tree_paths`가 내보낸다. 패키지가 이미 4단계 경로를 만들고 있어
  FLTK 쪽에 새로운 것은 없었다.
- **라벨을 파싱하지 않는다.** 캐시의 컬럼마다 라벨을 다시 만들어 문자열이 같은 것을
  찾는다. 파싱 규칙을 만들면 컬럼명에 공백이 있을 때 틀리고, 생성 규칙과 어긋날 수 있다.
- **열린 경로 복원을 일반화했다.** `open_package_tree_paths`의 하드코딩된
  `"Packages/"` 접두사를 걷어냈다. 이게 없으면 필터 한 글자에 펼친 테이블이 접힌다.
- **더블클릭은 계속 Browse.** 확장은 `→`와 화살표만. 가장 자주 쓰는 동작을 바꾸지 않았다.
- **메타데이터 새로고침은 캐시를 버린다.** `ALTER TABLE` 뒤에도 옛 컬럼을 보여주지
  않기 위해서다. 다음 확장 때 한 번 더 조회하면 그만이다.

**남은 것**: 인덱스·제약 자식 노드는 범위 밖(사용자 결정). 컬럼 노드에는 컨텍스트
메뉴가 없다 — 카탈로그가 설명하거나 DROP할 수 있는 객체가 아니다.
...
- **DataGrip**: 트리 노드, 구조 뷰, 자동완성 항목 옆에 객체·컬럼 코멘트를 보여준다.
- **현재 상태**: **어디에서도 읽지 않는다.** `TableColumnDetail`
  (`src/db/query/executor.rs:11270`)의 필드는 이름·타입·길이·정밀도·NULL 허용·
  기본값·PK 여부뿐이고 코멘트 자리가 없다. 메타데이터 조회 SQL 어디에도
  `ALL_COL_COMMENTS` / `USER_TAB_COMMENTS` / `COLUMN_COMMENT`가 없다. 따라서
  `View Structure`, 27번이 붙인 트리 컬럼 노드, IntelliSense 항목 어디에도
  코멘트가 뜨지 않는다.
- **왜 여기 있나**: 국내 스키마에서 컬럼명이 `CUST_NM`·`RGST_DTM` 같은 축약인 경우가
  많아 **코멘트가 사실상 진짜 이름**이다. 그게 안 보이면 컬럼을 고르려고 매번
  별도 쿼리(`ALL_COL_COMMENTS`)를 친다 — 우회 비용이 낮지 않고 빈도가 높다.
- **난이도**: 소~중. 표시할 자리는 이미 셋 다 있다(구조 뷰 컬럼 하나 추가, 트리
  컬럼 노드 라벨, 완성 항목의 상세 텍스트). 드는 비용은 **백엔드별 조회 확장**이다 —
  Oracle은 `ALL_COL_COMMENTS`/`ALL_TAB_COMMENTS` 조인, MySQL 계열은
  `information_schema.COLUMNS.COLUMN_COMMENT`가 이미 같은 행에 있다. 27번의
  `ObjectCache::table_columns` 캐시를 그대로 태우면 추가 왕복이 없다.
  코멘트 **편집**(`COMMENT ON ...`)은 37번(구조 변경 DDL)에 속한다.

### 35. 셀 값 ↔ 파일 (LOB을 파일로 저장 / 파일을 셀에 적재)

- **DataGrip**: 그리드 셀 우클릭 → Save LOB to file / Load file into cell.
  BLOB은 이미지 미리보기도 붙는다.
- **현재 상태**: 없다. `src/ui` 전역에 셀 → 파일, 파일 → 셀 진입점이 없다.
  4번이 값 창으로 **텍스트** 읽기·편집까지 뚫었지만, 거기서도 BLOB은 명시적으로
  범위 밖이다("그리드가 보여주는 값이 자리표시자라 어떤 비교도 옳을 수 없다").
- **왜 여기 있나**: 4번이 남긴 구멍의 정확한 형태가 이것이다. DataGrip은 BLOB을
  **화면에서 편집하지 않고 파일로 주고받는 것**으로 푼다. 즉 이 항목은 4번의
  잔여물이 아니라 **BLOB을 다루는 유일한 실용적 방법**이다. 저장 쪽만 해도
  "첨부파일 컬럼에서 파일 하나 꺼내기"가 되고, 지금은 그게 아예 불가능하다.
- **난이도**: 소(저장) / 중(적재). 저장은 값 창에 버튼 하나 + 기존
  `FileActionResult` 쓰기 경로 재사용이다. 적재가 비싼 이유는 따로 있다 —
  Oracle 편집 경로가 **리터럴 조립**이라(4번의 `TO_CLOB` 분할, `oracle_text_literal`)
  바이너리는 `HEXTORAW` 조각내기가 필요하고, 그보다 **바인드 경로로 가는 편이
  옳다**. 31번이 세션 바인드 파이프라인을 이미 뚫어 놨으므로 그쪽을 재사용할지가
  설계 결정이다. 크기 상한과 "얼마나 큰 파일까지 허용할지"도 정해야 한다.

### 36. 결과 다시 읽기 (Reload)

- **DataGrip**: 결과 탭의 새로고침 버튼(`Ctrl+F5`)이 **같은 쿼리를 그대로 다시**
  던진다. 데이터가 바뀌었는지 보는 가장 흔한 동작.
- **현재 상태**: 없다. `src/ui/table_browse.rs`에 refresh/reload 문자열이 하나도
  없고, 메뉴의 새로고침은 `Tools > Refresh Objects`(메타데이터 트리) 하나뿐이다
  (`menu.rs:548`). 지금 결과를 다시 읽는 방법은 (a) 에디터로 돌아가 다시 실행,
  (b) 필터 바가 있으면 필터를 다시 적용(= 재조회를 부수효과로 이용) 뿐이다.
- **왜 여기 있나**: 특히 **테이블 브라우징 탭**에서 아프다. 트리에서 연 탭에는
  돌아갈 에디터 문장이 없어서, 방금 커밋한 행을 확인하려면 탭을 닫고 다시 열어야
  한다. 빈도가 매우 높고 우회가 어색하다.
- **난이도**: 소. 재조회 파이프라인은 이미 완성돼 있다 — 필터 적용이 하는 일이
  정확히 "같은 논리 SQL을 현재 페이지로 다시 던지기"다(부록 A.1의
  `build_page_sql` 경로). 필터 입력을 바꾸지 않고 그 경로만 부르면 된다.
  주의할 것은 **재조회 불가능한 결과에는 버튼이 뜨면 안 된다**는 것(1번의 게이트가
  이미 그 판정을 갖고 있다)과, 편집 중이면 25번·6번과 같은 이유로 거절해야
  한다는 것이다.
...
#### 구현 상태 (2026-08-08) — 완료 (비정상 종료 후에만 제안)

| 조각 | 위치 |
| --- | --- |
| 스냅샷 모델·원자적 쓰기 (테스트 7개) | `src/utils/local_history.rs` |
| 주기적 기록 | `MainWindow::write_unsaved_tab_snapshot` (유휴 폴링 5초마다) |
| 정상 종료 시 삭제 | `finish_application_exit` |
| 복원 제안 | `MainWindow::offer_unsaved_tab_restore` (`src/app.rs` 시작 경로) |
| 캡쳐 | `capture_feature_tour restore-tabs` |

설계상 결정 네 가지:

- **스냅샷의 존재 자체가 신호다.** 정상 종료가 파일을 지우므로, 시작 시 파일이 있다 =
  지난 실행이 죽었다. 정상 종료 사용자는 아무 변화도 느끼지 않는다.
- **바뀐 게 없으면 디스크를 만지지 않는다.** 더티 탭만 모아 해시를 내고 직전 값과
  같으면 직렬화조차 하지 않는다. 큰 스크립트를 5초마다 인코딩하지 않기 위해서다.
- **원자적 쓰기.** 임시 파일 → `sync_all` → `chmod 0600` → `rename`.
  쓰는 도중에 죽는 순간이 바로 이 파일이 필요해지는 순간이다.
- **8 MB를 넘는 탭은 자르지 않고 건너뛴다**(로그에 이름을 남긴다). 조용히 잘린
  스크립트를 복원해 주는 쪽이 복원하지 않는 것보다 나쁘다.

**남은 것**: 복원된 탭은 커넥션 바인딩을 되살리지 않는다. 로컬 히스토리(버전별
되돌리기)도 범위 밖 — 이번 것은 데이터 손실 방지 장치다.
...
### 37. 객체 생성·구조 변경 DDL (Create / Modify Table / Rename / Comment)

- **현재 상태**: 5번이 `Drop`/`Truncate`까지 왔고 거기서 멈춰 있다. 소스에
  `Rename` · `Create Table` · `Modify Table` · `Alter` 라벨이 여전히 없다
  (`src/ui/object_browser.rs` 재확인). 34번이 지목한 `COMMENT ON`도 여기 속한다.
- **왜 여기 있나**: 5번이 "1차 범위 밖"으로 남긴 것을 항목 없이 두면 추적이 끊긴다.
  다만 Tier 0이 아닌 이유는 그대로다 — 스키마를 만드는 일은 SQL을 직접 쓰는 것이
  자연스럽고, 이 앱은 그 경로가 이미 좋다.
- **범위를 나누면 값이 크게 갈린다**:
  - **Rename** — 작다. 5번의 `DestructiveObjectAction`이 만든 "확인 다이얼로그 →
    에디터에 문장 → 일반 실행" 틀을 그대로 쓴다. 이름 입력 하나가 추가될 뿐이다.
  - **Comment 편집** — 작다. 같은 틀에 텍스트 입력 하나.
  - **Modify Table / Create Table** — 크다. 컬럼·타입·제약·인덱스를 다루는 폼과
    "현재 상태 → 목표 상태"의 차이를 `ALTER`로 바꾸는 계산이 필요하다.
- **난이도**: 소(Rename · Comment) / 대(Modify · Create).

### 38. 단일 레코드(Transpose) 뷰

- **현재 상태**: 없다(소스 전역에 `transpose` 없음). 4번의 값 창은 **셀 하나**를
  키우고, 행 전체를 세로로 보는 수단은 없다.
- **영향**: 컬럼이 40개인 테이블에서 한 행을 읽으려면 가로 스크롤을 끝까지 민다.
  6번(컬럼 숨김/순서)이 이걸 완화하지만 대체하지는 못한다 — 숨기는 것은 "이 결과
  전체"에 걸리는 설정이고, Transpose는 "지금 이 한 행"을 보는 동작이다.
- **난이도**: 소~중. 값은 이미 `full_data`에 있고 읽기 전용으로 시작하면 새 평가나
  조회가 없다. 다만 **편집 가능한 Transpose는 다르다** — 편집 세션이 행×컬럼
  인덱스로 상태를 짝짓고 있어서(6번·25번이 편집과 배타적으로 간 이유와 같다)
  1차는 읽기 전용이 옳다.

### 39. 그리드 편집 부분 되돌리기 (Revert Selected)

- **DataGrip**: 스테이징된 변경을 셀/행 단위로 되돌린다(`Ctrl+Alt+Z`).
- **현재 상태**: **전부 버리기만 있다.** `cancel_edit_mode`
  (`src/ui/result_table.rs:7637`)가 편집 모드를 끄면서 스테이징된 변경을 통째로
  버리는 것이 유일한 경로다. 셀 하나만 원래 값으로 돌릴 방법이 없다.
- **왜 여기 있나**: 데이터 안전성 쪽이다. 20행을 고치다 한 셀을 잘못 건드리면
  선택지가 "잘못된 채로 저장" 아니면 "19행의 옳은 작업까지 버리기" 둘뿐이다.
  빈도는 낮지만 걸리면 손실이 확실하다.
- **난이도**: 소. 원래 값은 이미 갖고 있다 — 저장 시 `original_value_predicate`가
  쓰는 그 값이다(4번 참조). 되돌리기는 그 값을 화면에 다시 넣고 해당 셀의 더티
  표시를 지우는 것이고, 행의 마지막 더티 셀이 사라지면 행도 깨끗해진다.

### 40. 객체 트리 다중 선택 + 배치 작업

- **현재 상태**: 없다. 트리는 한 번에 한 노드만 다루고(다중 선택 개념이
  `object_browser.rs`에 없다), 모든 컨텍스트 메뉴 액션이 단일 객체를 받는다.
- **영향**: 테이블 12개의 DDL을 뽑거나 임시 테이블 여러 개를 지우는 일이 12번의
  반복이 된다.
- **난이도**: 중. FLTK `Tree`의 다중 선택 자체는 지원되지만, 액션 쪽이 전부
  `ObjectItem` 하나를 전제로 짜여 있어 시그니처가 넓게 바뀐다. 그리고
  **5번이 세운 원칙(미리보기 후 실행)을 배치에서도 지켜야 한다** — 문장 N개를
  한 화면에 보여주고 한 번에 에디터로 보내는 형태가 맞다.

### 41. SQL 인스펙션 (미해결 참조 경고)

- **DataGrip**: 없는 테이블·컬럼에 밑줄을 긋고 `Alt+Enter`로 고칠 거리를 준다.
  실행하기 전에 오타를 안다.
- **현재 상태**: 없다. 하이라이터는 문법 색칠까지고
  (`src/ui/syntax_highlight.rs`), 진단·밑줄·퀵픽스 개념이 없다. IntelliSense는
  **제안**만 하고 이미 쓰인 이름이 실재하는지는 보지 않는다.
- **왜 여기 있나**: 재료가 이미 다 있다는 점에서 아깝다 — 스키마 카탈로그도,
  커서 위치의 문맥 판정도, 55회 이상 정밀도 감사를 거친 스코프 해석기도 있다.
  없는 것은 "해석 실패를 경고로 바꾸는 층" 하나다.
- **난이도**: 중~대. 기술적 난이도보다 **오탐 비용**이 문제다. 동적 SQL, 아직
  만들지 않은 임시 객체, 다른 스코프의 객체, 캐시가 비어 있는 상태에서 잘못
  긋는 밑줄은 IntelliSense의 잘못된 제안보다 훨씬 성가시다. 넣는다면
  **카탈로그가 확실히 로드된 스코프의 확실한 미존재만** 긋는 보수적 규칙으로
  시작해야 한다. 이 앱이 1번·12번에서 반복해 택한 판단(추측하지 않는다)과 같은 선.
...
- **기타 추가 (2026-08-08 3차 재조사, 전부 없음을 확인)**: 커넥션 프로파일 복제
  (지금은 새로 만들어 다시 입력), 상태 바의 **상시** 트랜잭션/오토커밋 표시
  (`StatusBarWidget`은 커넥션 표시등·상태 텍스트·선택 집계·추가 건수뿐이고
  오토커밋은 바뀔 때 지나가는 메시지로만 보인다 — 다만 탭을 닫을 때
  `Commit and Close`가 물어보므로 미커밋 상태로 조용히 잃는 경로는 없다),
  그리드의 행 번호로 이동, 에디터의 현재 실행 문장 강조. 넷 다 작지만 각각이
  독립적으로는 흐름을 막지 않아 항목으로 세우지 않았다.
...
| ✅ | ~~25. 셀 값으로 빠른 필터~~ | **완료** — 단 필터 바와 **배타적인 로컬 필터**로 설계 변경 |
| ✅ | ~~26. 객체 트리 Export / Import~~ | **완료** — 진행률·취소는 범위 밖 |
| ✅ | ~~24. 결과 정렬 수단~~ | **완료** — 하이브리드. 부록 B.2 결함 5도 함께 닫힘 |
| ✅ | ~~16. 미저장 탭 복원~~ | **완료** — 비정상 종료 후에만 제안 |
| ✅ | ~~6. 컬럼 숨김/순서~~ · ~~27. 트리 컬럼 노드~~ | **완료** — 고정(pin)과 인덱스/제약 노드는 범위 밖 |
| 1 | 36. 결과 다시 읽기 (Reload) | 재조회 파이프라인이 이미 있다. 남은 일이 버튼과 게이트뿐 |
| 2 | 34. 테이블/컬럼 코멘트 | 표시할 자리 셋이 이미 있고 27번 캐시에 얹으면 추가 왕복이 없다 |
| 3 | 39. 편집 부분 되돌리기 | 원래 값을 이미 들고 있다. 데이터 안전성 |
| 4 | 37. Rename · Comment 편집만 | 5번이 만든 "미리보기 후 실행" 틀 재사용. Modify/Create는 분리 |
| 5 | 35. 셀 값 ↔ 파일 | 저장부터. 적재는 바인드 경로 설계 결정이 선행 |
| 6 | 38. Transpose 뷰 | 읽기 전용으로 시작 |
| — | 40 · 41 · 37(Modify/Create) | 비용이 크거나(41 오탐, 37 DDL 계산) 시그니처가 넓게 바뀐다(40) |

### 완료 항목의 남은 조각 (2026-08-08 재확인)

완료로 적힌 항목들이 각자 "남은 것"을 본문에 적어 뒀는데, 흩어져 있어 추적이
어려웠다. 한 표로 모으고 **소스에서 아직 없는 것이 맞는지 다시 확인**했다.
전부 여전히 열려 있다.

| 항목 | 남은 조각 | 재확인 근거 | 처리 |
| --- | --- | --- | --- |
| 2. 내보내기 | Excel(xlsx) | `xlsx`/`Excel` 문자열 없음 | 새 크레이트 필요. 그대로 보류 |
| 2. 내보내기 | 파일 대상의 GUI 자동 검증 | macOS 네이티브 저장 패널 | 검증 불가. 클립보드 경로로 대체 확인 |
| 3. 임포트 | 비UTF-8 인코딩, Markdown/HTML 왕복 손실 | 본문 기록 | 의도된 범위 밖 |
| 4. 값 뷰어 | Transpose 뷰 | `transpose` 없음 | **→ 신규 38** |
| 4. 값 뷰어 | 값 창 내부 검색 | `value_viewer.rs` | 소. 38과 함께 |
| 4. 값 뷰어 | BLOB 편집 | — | **→ 신규 35** (파일로 주고받는 형태) |
| 5. Drop/Truncate | Rename, Modify Table | `Rename`/`Modify` 라벨 없음 | **→ 신규 37** |
| 6. 컬럼 배치 | 고정(pin) | `frozen`/`pinned` 없음 | FLTK 커스텀 드로잉 필요. 보류 |
| 10. Go to Declaration | 서버 전역 검색 | `object_search*.rs`에 `ALL_OBJECTS` 없음 | 현재 스코프 캐시만 본다 |
| 11. 커넥션 색상 | 결과 탭·트리에 색 칠하기 | 상태바·탭 라벨 세 곳뿐 | 소. 남겨 둠 |
| 12. 실행 계획 | `EXPLAIN FORMAT=JSON` 트리, Note 각주 | 본문 기록 | 버전별 구조 차이로 범위 밖 |
| 16. 탭 복원 | 커넥션 바인딩 복원 | `local_history.rs`에 커넥션 없음 | 복원된 탭은 바인딩이 비어 있다 |
| 25. 값 필터 | 범위·부분 문자열 조건 | 값 일치만 | 부분 문자열은 11-b가 대신한다 |
| 26. 트리 Export | 진행률·취소 | `render_table_export`에 없음 | 큰 테이블에서 되돌릴 수 없다 |
| 27. 트리 컬럼 | 인덱스·제약·트리거 자식 노드 | `ObjectItem`에 없음 | 사용자 결정으로 범위 밖 |
| 28. 에디터 편의 | 코드 폴딩 | — | FLTK에 폴딩 개념 없음. Tier 2 유지 |

새로 추가한 34·36·39·40·41은 이 표의 잔여물이 아니라 **이번 재조사에서 처음
확인한 구멍**이다.
...
- 2026-08-08 3차 재조사도 앞선 재조사와 같은 기준으로 **진입점의 존재 여부만**
  봤다. 34~41번은 해당 문자열·필드·핸들러가 소스에 아예 없다는 점에서 "없다"가
  확실하다. 확실하지 않은 것은 **비용 쪽**이다 — 특히 34번은 백엔드별 메타데이터
  조회를 넓히는 일이라 카탈로그 뷰 권한이 없는 계정에서 어떻게 되는지(조회가
  실패하면 컬럼 목록 자체가 안 뜨는가)를 확인하지 않았고, 41번의 오탐 비율은
  실제로 붙여 보기 전에는 추정일 뿐이다.
- 이번 재조사에서 **없는 줄 알았다가 있는 것으로 확인해 제외한 항목이 넷**이다
  (그리드 붙여넣기, Copy Name, 히스토리 검색, Commit and Close). 조사 범위의
  "제외 추가" 절에 근거와 함께 남겼다 — 다음 재조사에서 같은 것을 다시 세우지
  않기 위해서다.
...
>
> **2026-08-08 후속 — 되살아났다.** [24번](#24-결과-정렬-수단-헤더-정렬-제거-이후)이
> B.3의 하이브리드 안 그대로 구현되면서 `src/ui/grid_sort.rs`와 헤더 정렬이 커밋
> `f635179^`에서 복구됐다. 아래 (a)/(b) 경계가 실제 게이트가 됐고, **결함 5도 이때
> 닫혔다** — 리다이렉트 설치가 `build_filter_bar` 한 곳으로 옮겨져 테이블 브라우징
> 탭도 서버 `ORDER BY`로 간다(제거 이전에는 브라우징 탭이 로컬 정렬로 떨어졌다).
>
> 아래는 제거 이전의 기록이며, 설계 근거로 그대로 유효하다.
