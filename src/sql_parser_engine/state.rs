...
pub(crate) struct SplitState {
    ...
    /// The statement mentioned MLE, so `{{` may open an inline foreign body.
    saw_mle_language_marker: bool,
    /// True while the next character starts a fresh line inside a foreign-source
    /// body; only there can a lone `/` close the body.
    pub(crate) foreign_source_at_line_start: bool,
...
impl SplitState {
    ...
    pub(crate) fn prepare_splitter_line_boundary(&mut self, line: &str) {
        self.close_foreign_module_source_on_terminator_line(line);
    ...
    /// A bare `/` line ends an MLE module's foreign-source body. Leaving the
    /// foreign mode here - before the line boundary is classified - lets the
    /// ordinary slash-terminator path close the statement.
    pub(crate) fn close_foreign_module_source_on_terminator_line(&mut self, line: &str) {
        if self.lex_mode == LexMode::ForeignModuleSource && is_bare_slash_terminator_line(line) {
            self.close_foreign_module_source();
        }
    }

    /// Leaves an MLE module body. The statement is complete at that point, so the
    /// CREATE tracking is cleared too - otherwise a caller that never sees a
    /// statement boundary (the whole-text lexical scan has no `/` handling of its
    /// own) would carry `MLE MODULE` into the next unit and read its `AS` as the
    /// start of another foreign body.
    pub(crate) fn close_foreign_module_source(&mut self) {
        self.lex_mode = LexMode::Idle;
        self.foreign_source_at_line_start = false;
    ...
    pub(crate) fn in_mle_module_create(&self) -> bool {
        self.create_plsql_kind == CreatePlsqlKind::MleModule
    }

    /// True once the statement has named MLE, so a following `{{` opens an
    /// inline foreign-language body rather than being stray punctuation.
    pub(crate) fn allows_inline_foreign_source(&self) -> bool {
        self.saw_mle_language_marker
    }
    ...
    fn keep_semicolons_inside_create_body(&self) -> bool {
        self.in_java_source_create() || self.in_wrapped_create() || self.in_mle_module_create()
    }
    ...
    pub(crate) fn flush_token(&mut self) {
        ...
        self.track_foreign_source_body(upper);
    ...
    /// Opens the foreign-source region of an MLE unit. `CREATE MLE MODULE ...
    /// AS` is followed by JavaScript, not SQL, so from the `AS` onwards the
    /// lexer must stop applying SQL string, comment and terminator rules.
    fn track_foreign_source_body(&mut self, upper: &str) {
        if upper == "MLE" {
            self.saw_mle_language_marker = true;
            return;
        }

        if self.in_mle_module_create()
            && upper == "AS"
            && self.block_depth() == 0
            && self.paren_depth == 0
        {
            self.lex_mode = LexMode::ForeignModuleSource;
            self.foreign_source_at_line_start = false;
        }
    }
    ...
    pub(crate) fn reset_create_tracking_state(&mut self) {
        ...
        self.saw_compound_keyword = false;
        self.saw_trigger_alias_subject = false;
        self.saw_mle_language_marker = false;
        self.foreign_source_at_line_start = false;
    }
    ...
    fn track_create_plsql(&mut self, upper: &str) {
        ...
        if self.create_state == CreateState::AwaitingMleTarget {
            self.create_state = CreateState::None;
            if upper == "MODULE" {
                // Only the module form carries a foreign-source body; `CREATE MLE
                // ENV` is an ordinary statement.
                self.create_plsql_kind = CreatePlsqlKind::MleModule;
            }
            return;
        }
        ...
        if self.create_state == CreateState::AwaitingObjectType {
            match upper {
                ...
                "MLE" => {
                    self.create_state = CreateState::AwaitingMleTarget;
                    return;
                }
                "PROCEDURE" => {
