...
pub(crate) enum LexMode {
    ...
    /// Body of `CREATE MLE MODULE ... AS <source>`: foreign-language source, not
    /// SQL. Nothing inside it may be read as a SQL string, comment or
    /// terminator - only a line that is exactly `/` closes it.
    ForeignModuleSource,
    /// Inline `{{ ... }}` foreign-language body of an MLE call spec. Closed by
    /// the `}}` that balances the opening braces.
    ForeignInlineSource {
        brace_depth: usize,
    },
}
...
pub(crate) enum CreateState {
    ...
    AwaitingObjectType,
    AwaitingJavaTarget,
    /// Saw `CREATE [OR REPLACE] MLE`; the next word decides whether this is a
    /// module (a foreign-source body) or an environment (an ordinary
...
enum CreatePlsqlKind {
    ...
    Trigger(TriggerKind),
    JavaSource,
    /// `CREATE MLE MODULE <name> LANGUAGE <lang> AS <foreign source>`.
    MleModule,
    Wrapped,
}
