...
#[test]
fn lexical_spans_treat_sqlplus_prompt_payload_as_opaque_line_text() {
    let sql = concat!(
        "PROMPT [w10] don't stop; keep going -- not a SQL comment\n",
        "SELECT 1 AS \"it's\", q'\"double quoted q\"' AS q_value FROM dual;\n",
        "SELECT \"SELECT\" + \"order\" FROM quoted_words;"
    );
    let spans = lexical_spans(sql, false);

    let prompt_start = sql.find("PROMPT").expect("PROMPT line");
    let prompt_end = sql.find('\n').expect("PROMPT line end");
    assert!(spans.iter().any(|span| {
        span.kind == LexicalKind::LineComment
            && span.start == prompt_start
            && span.end == prompt_end
    }));

    for quoted in ["\"it's\"", "\"SELECT\"", "\"order\""] {
        let start = sql.find(quoted).expect("quoted identifier");
        assert!(spans.iter().any(|span| {
            span.kind == LexicalKind::QuotedIdentifier
                && span.start <= start
                && start + quoted.len() <= span.end
        }));
    }

    let q_quote = "q'\"double quoted q\"'";
    let q_quote_start = sql.find(q_quote).expect("q-quote");
    assert!(spans.iter().any(|span| {
        span.kind == LexicalKind::String
            && span.start <= q_quote_start
            && q_quote_start + q_quote.len() <= span.end
    }));
}

#[test]
fn lexical_spans_keep_oracle_final_hardcore_wave10_quotes_bounded() {
    let sql = include_str!("../../test/final_hardcore.sql");
    let spans = lexical_spans(sql, false);

    let prompt_apostrophe = sql.find("don't stop").expect("PROMPT apostrophe") + 3;
    assert_eq!(
        super::lexical_kind_at(&spans, prompt_apostrophe),
        Some(LexicalKind::LineComment),
        "PROMPT payload must not open a SQL string"
    );

    for quoted in ["\"it's\"", "\"trailing space \"", "\"SELECT\"", "\"order\""] {
        let start = sql.find(quoted).expect("quoted identifier");
        assert_eq!(
            super::lexical_kind_at(&spans, start),
            Some(LexicalKind::QuotedIdentifier),
            "{quoted} must start its own quoted-identifier span"
        );
        assert!(
            spans.iter().any(|span| {
                span.kind == LexicalKind::QuotedIdentifier
                    && span.start == start
                    && span.end == start + quoted.len()
            }),
            "{quoted} must close without contaminating following SQL"
        );
    }
}
