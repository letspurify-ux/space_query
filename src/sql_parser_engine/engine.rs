...
/// True when `line` is nothing but a `/` terminator. Foreign-language bodies use
/// this stricter rule than SQL statements do: a JavaScript line may legally
/// start with `/*` (a JSDoc banner) or a regular-expression literal, and the
/// server only ends the body on a bare slash.
pub(crate) fn is_bare_slash_terminator_line(line: &str) -> bool {
    line.trim() == "/"
}

/// Same test as [`is_bare_slash_terminator_line`], applied to the line that
/// starts at `start` inside `chars`.
fn line_at_is_bare_slash_terminator(chars: &[char], start: usize) -> bool {
    let mut saw_slash = false;
    for &ch in &chars[start..] {
        match ch {
            '\n' => break,
            '/' if !saw_slash => saw_slash = true,
...
impl SqlParserEngine {
    ...
    fn line_is_mysql_server_statement(&self, line: &str) -> bool {
        ...
        let Some((first, first_end)) = next_meaningful_word(line, 0) else {
            return false;
        };
        if matches!(
        ...
        first.eq_ignore_ascii_case("GET")
            && next_meaningful_word(line, first_end).is_some_and(|(next, _)| {
                matches!(
                    next.to_ascii_uppercase().as_str(),
                    "CURRENT" | "DIAGNOSTICS" | "STACKED"
                )
            })
    }

    pub(crate) fn starts_with_mysql_get_diagnostics_context(&self) -> bool {
        self.state.mysql_mode
            && (self.starts_with_keyword_pair("GET", "CURRENT")
                || self.starts_with_keyword_pair("GET", "DIAGNOSTICS")
                || self.starts_with_keyword_pair("GET", "STACKED"))
    }
        ...
        H: FnMut(&[char], usize, usize, LexicalKind),
    {
        ...
        while i < len {
            ...
            match &self.state.lex_mode {
                ...
                LexMode::ForeignModuleSource => {
                    // Foreign-language source: hand every character through
                    // untouched. Only a line that is exactly `/` closes the body,
                    // and that slash is left for the ordinary terminator path.
                    if self.state.foreign_source_at_line_start
                        && line_at_is_bare_slash_terminator(chars, i)
                    {
                        self.state.close_foreign_module_source();
                        continue;
                    }
                    on_lexical_span(chars, i, i + 1, LexicalKind::String);
                    self.append_current_char(c);
                    self.state.foreign_source_at_line_start = c == '\n';
                    i += 1;
                    continue;
                }
                LexMode::ForeignInlineSource { brace_depth } => {
                    let mut brace_depth = *brace_depth;
                    on_lexical_span(chars, i, i + 1, LexicalKind::String);
                    self.append_current_char(c);
                    match c {
                        '{' => brace_depth += 1,
                    ...
                    self.state.lex_mode = if brace_depth == 0 {
                        LexMode::Idle
                    } else {
                        LexMode::ForeignInlineSource { brace_depth }
                    };
            ...
            if c == '{'
                && next == Some('{')
                && self.state.allows_inline_foreign_source()
                && self.state.token.is_empty()
            {
                // `AS MLE LANGUAGE JAVASCRIPT {{ ... }}` embeds the routine body
                // inline. Everything up to the balancing `}}` is foreign source.
                on_lexical_span(chars, i, i + 2, LexicalKind::String);
                self.append_current_char(c);
                self.append_current_char('{');
                self.state.lex_mode = LexMode::ForeignInlineSource { brace_depth: 2 };
                i += 2;
                continue;
            }
        ...
        G: FnMut(&[char], usize),
    {
        ...
        self.state
            .close_foreign_module_source_on_terminator_line(line);
        ...
        F: FnMut(&[char], usize, usize, LexicalKind),
    {
        ...
        while line_start < scratch_chars.len() {
            let line_end = scratch_chars[line_start..]
                .iter()
                .position(|ch| *ch == '\n')
                .map_or(scratch_chars.len(), |offset| line_start + offset);
            let chunk_end = usize::from(line_end < scratch_chars.len())
                .saturating_add(line_end)
                .min(scratch_chars.len());
            let line: String = scratch_chars[line_start..line_end].iter().collect();
            let line_started_with_empty_current = self.current_is_empty();
            let current_has_only_sql_comments =
                next_meaningful_word(self.current.as_str(), 0).is_none();
            let current_is_effectively_empty =
                line_started_with_empty_current || current_has_only_sql_comments;
            let line_started_in_with_waiting_main_query =
                self.state.in_with_plsql_declaration()
                    && self.state.with_clause_waiting_main_query()
                    && self.state.block_depth() == 0
                    && self.state.paren_depth() == 0;
            ...
            let line_boundary_action = self
                .state
                .line_boundary_action_for_line(&line, current_is_effectively_empty);
            if self.apply_line_boundary_action(line_boundary_action) {
            ...
            let line_starts_at_statement_boundary = self.state.is_idle()
                ...
                && self.state.paren_depth() == 0
                && !self.state.in_with_plsql_declaration()
                && current_is_effectively_empty;
            let sqlplus_remark_line = sql_text::is_sqlplus_remark_comment_line(&line);
            let auto_terminated_tool_command = sqlplus_remark_line
                || (sql_text::is_auto_terminated_tool_command(&line)
                    && !self.line_is_mysql_server_statement(&line));

            if (line_starts_at_statement_boundary
                || (line_started_in_with_waiting_main_query && self.state.is_idle()))
                && auto_terminated_tool_command
            {
                let is_comment_like_tool_line = sql_text::first_meaningful_word(&line)
                    .is_some_and(|word| word.eq_ignore_ascii_case("PROMPT"));
                if is_comment_like_tool_line {
                    on_lexical_span(
                        &scratch_chars,
                        line_start,
                        line_end,
            ...
            let mut offset_lexical_span =
                |_: &[char], start: usize, end: usize, kind: LexicalKind| {
                    on_lexical_span(
                        &scratch_chars,
            ...
            self.process_chars_with_observer(
                &scratch_chars[line_start..chunk_end],
                &mut on_symbol,
                &mut on_statement_boundary,
                &mut offset_lexical_span,
            );
            self.state.clear_skip_next_end_label_token();
