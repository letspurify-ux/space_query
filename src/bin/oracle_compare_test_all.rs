...
fn child_run(
...
) -> Result<DriverRunSnapshot, String> {
    ...
    let sql = if path == "__cleanup__" {
        ...
        let mut body = serialize_driver_divergent_select_items(&body);
        if let Some(layout) = format_layout {
            body = SqlEditorWidget::format_script_for_harness(&body, DatabaseType::Oracle, layout);
        }
        if is_final_sql {
            body
        } else {
...
fn main() {
    ...
    if args.first().map(String::as_str) == Some("--child") {
        if args.len() < 3 || args.len() > 6 {
            eprintln!(
                "usage: oracle_compare_test_all --child <oci|thin> <path> \
        ...
        match child_run(mode, &args[2], format_layout) {
            Ok(snapshot) => {
