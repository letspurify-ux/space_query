...
/// Foreign-language source embedded in SQL: the body of an MLE module or of an
/// inline `{{ ... }}` call spec. Rendered inert, like a literal, because no SQL
/// rule applies inside it.
pub const STYLE_FOREIGN_SOURCE: char = 'O';
...
pub fn create_style_table_with(profile: FontProfile, size: u32) -> Vec<StyleTableEntry> {
    vec![
        ...
        // O - Foreign-language source (muted orange, italic)
        StyleTableEntry {
            color: Color::from_rgb(190, 150, 140),
            font: profile.italic,
            size: size as i32,
        },
    ]
...
pub enum LexerState {
    ...
    InBacktickQuote,
    InBracketQuote,
    /// Inside foreign-language source. `inline_brace_depth` is 0 for an MLE
    /// module body (closed by a line that is only `/`) and otherwise counts the
    /// open braces of an inline `{{ ... }}` body.
    InForeignSource {
        inline_brace_depth: usize,
    },
    /// An MLE call-spec header ended the chunk, so a `{{` on a following line
    /// still opens an inline foreign body.
    AwaitingInlineForeignSource,
}
...
impl SqlHighlighter {
    ...
    pub fn entry_state_from_continuation_style(&self, style: char) -> LexerState {
        match style {
            ...
            STYLE_FOREIGN_SOURCE => LexerState::InForeignSource {
                inline_brace_depth: 0,
            },
    ...
    fn generate_style_bytes_with_state_impl(
        ...
        match initial_state {
            ...
            LexerState::InForeignSource { inline_brace_depth } => {
                let scan = if inline_brace_depth == 0 {
                    scan_until_foreign_module_source_end(bytes, idx)
                } else {
                    scan_until_inline_foreign_source_end(bytes, idx, inline_brace_depth)
                };
                match scan {
                    ScanResult::Closed { next_idx } => {
                        idx = next_idx;
                        styles[..idx].fill(STYLE_FOREIGN_SOURCE as u8);
                    }
                    ScanResult::Unterminated { state, .. } => {
                        styles[..].fill(STYLE_FOREIGN_SOURCE as u8);
                        return state;
                    }
                }
            }
            LexerState::AwaitingInlineForeignSource => {
                scan_context.saw_mle_keyword = true;
            }
            LexerState::Normal => {}
        }
        ...
        while let Some(&byte) = bytes.get(idx) {
            ...
            if sql_text::is_identifier_start_byte(byte) {
                ...
                // MLE units embed another language. Track the header words, and
                // when the `AS` that opens the body arrives, hand the rest of the
                // statement to the foreign-source scanner so no SQL rule paints
                // anything inside it.
                match folded_word.upper() {
                    "MLE" => scan_context.saw_mle_keyword = true,
                    "MODULE" if scan_context.saw_mle_keyword => {
                        scan_context.mle_module_header = true;
                    }
                    _ => {}
                }
                if matches!(folded_word.upper(), "AS" | "IS") && scan_context.mle_module_header {
                    styles[start..idx].fill(STYLE_KEYWORD as u8);
                    scan_context.mle_module_header = false;
                    scan_context.saw_mle_keyword = false;
                    match scan_until_foreign_module_source_end(bytes, idx) {
                        ScanResult::Closed { next_idx } => {
                            styles[idx..next_idx].fill(STYLE_FOREIGN_SOURCE as u8);
                            idx = next_idx;
                            continue;
                        }
                        ScanResult::Unterminated { state, .. } => {
            ...
            // `AS MLE LANGUAGE JAVASCRIPT {{ ... }}` embeds the routine body
            // inline; everything up to the balancing `}}` is foreign source.
            if byte == b'{' && bytes.get(idx + 1) == Some(&b'{') && scan_context.saw_mle_keyword {
                scan_context.saw_mle_keyword = false;
                match scan_until_inline_foreign_source_end(bytes, idx, 0) {
                    ScanResult::Closed { next_idx } => {
                        styles[idx..next_idx].fill(STYLE_FOREIGN_SOURCE as u8);
                        idx = next_idx;
                        scan_context.clear_prev_token();
                        continue;
                    }
                    ScanResult::Unterminated { state, .. } => {
                        styles[idx..].fill(STYLE_FOREIGN_SOURCE as u8);
        ...
        if exit_state == LexerState::Normal && scan_context.saw_mle_keyword {
            // The chunk ended inside an MLE call-spec header; remember it so a
            // `{{` on the next line still opens the inline body.
            return LexerState::AwaitingInlineForeignSource;
        }
...
struct HighlightScanContext<'a> {
    /// The statement named MLE, so `{{` opens an inline foreign body.
    saw_mle_keyword: bool,
    /// The statement is `CREATE ... MLE MODULE`, so its `AS` opens a body of
    /// foreign-language source.
    mle_module_header: bool,
...
/// Scans an MLE module body. The body is foreign-language source and only a line
/// that is exactly `/` closes it, so nothing inside is read as SQL.
fn scan_until_foreign_module_source_end(bytes: &[u8], mut idx: usize) -> ScanResult {
    loop {
        // Position `idx` at the start of a line, then test that line.
        let line_start = idx;
        ...
        if bytes.get(scan) == Some(&b'/') {
            ...
            if bytes
                .get(after)
                .is_none_or(|byte| is_line_terminator(*byte))
            {
        ...
        // Not a terminator line: consume it and continue with the next one.
        while let Some(&byte) = bytes.get(idx) {
            idx += 1;
            if byte == b'\n' {
                break;
        ...
        if bytes.get(idx).is_none() {
            return ScanResult::Unterminated {
                next_idx: bytes.len(),
                state: LexerState::InForeignSource {
                    inline_brace_depth: 0,
                },
            };
        }
...
/// Scans an inline `{{ ... }}` foreign body from `idx`, which must sit on the
/// first `{`. Braces are balanced, so a nested JavaScript object literal does not
/// end the body early.
fn scan_until_inline_foreign_source_end(
    bytes: &[u8],
    mut idx: usize,
    mut depth: usize,
) -> ScanResult {
    while let Some(&byte) = bytes.get(idx) {
        match byte {
            ...
            b'}' => {
                ...
                if depth == 0 {
                    return ScanResult::Closed { next_idx: idx + 1 };
                }
            }
            _ => {}
        }
    ...
    ScanResult::Unterminated {
        next_idx: bytes.len(),
        state: LexerState::InForeignSource {
            inline_brace_depth: depth.max(1),
        },
    }
}
