...
pub struct CursorContext {
    ...
    /// Visible Oracle row sources that use a `VERSIONS ... BETWEEN` clause.
    /// Kept separately from catalog tables so completion can expose the
    /// `VERSIONS_*` pseudocolumns only through the owning row-source alias.
    oracle_flashback_versions_sources: Vec<ScopedTableRef>,
...
impl CursorContext {
    pub(crate) fn qualifier_has_oracle_flashback_versions(&self, qualifier: &str) -> bool {
        resolve_qualifier_table_refs(qualifier, &self.tables_in_scope)
            .into_iter()
            .any(|resolved| {
                self.oracle_flashback_versions_sources.iter().any(|source| {
                    resolved.depth == source.depth
                        && resolved.is_cte == source.is_cte
                        && normalize_identifier_for_lookup(&resolved.name)
                            == normalize_identifier_for_lookup(&source.name)
                        && resolved
                            .alias
                            .as_deref()
                            .map(normalize_identifier_for_lookup)
                            == source.alias.as_deref().map(normalize_identifier_for_lookup)
                })
            })
    }
}
...
pub(crate) fn analyze_cursor_context_arc(
    ...
    let visible_scope_ids: HashSet<usize> =
        parse_result.visible_scope_chain.iter().copied().collect();
    let oracle_flashback_versions_sources = parse_result
        .parsed_tables
        .iter()
        .filter(|entry| {
            entry.has_oracle_flashback_versions && visible_scope_ids.contains(&entry.scope_id)
        })
    ...
    CursorContext {
        ...
        oracle_flashback_versions_sources,
...
fn is_set_statement_embedded_statement_head(tokens: &[SqlToken], idx: usize) -> bool {
    let words = significant_statement_tokens_before_cursor(tokens, idx.saturating_add(1))
        .into_iter()
    ...
    matches!(
        words.as_slice(),
        [set, statement, .., for_kw, head]
            if set == "SET"
                && statement == "STATEMENT"
                && for_kw == "FOR"
                && matches!(
                    head.as_str(),
                    "CALL" | "DELETE" | "INSERT" | "REPLACE" | "SELECT" | "UPDATE" | "WITH"
                )
    )
}
...
fn ddl_alter_table_column_target(sig: &[&SqlToken]) -> Option<String> {
    ...
    let add_scope_for_column = op == Some("ADD")
        && sig[after_table..]
            .windows(2)
            .any(|window| token_is_word(window[0], "SCOPE") && token_is_word(window[1], "FOR"))
        && (token_is_symbol(prev, "(") || token_is_symbol(prev, ","));
    let at_existing_column = add_scope_for_column
        || match op? {
...
struct ParsedTableEntry {
    ...
    has_oracle_flashback_versions: bool,
}
        ...
        match token {
                    ...
                    if start_idx <= idx {
                        ...
                        )) = parse_subquery_alias(tokens, idx + 1)
                        {
                            ...
                            all_tables.push(ParsedTableEntry {
                                ...
                                has_oracle_flashback_versions: false,
                            });
                            idx = next_idx;
            ...
            SqlToken::Word(word) => {
                ...
                let current_statement_kind = depth_frames
                    ...
                    .map(|frame| frame.statement_kind)
                    .unwrap_or(StatementKind::Unknown);
                let set_statement_embedded_head =
                    depth == 0 && is_set_statement_embedded_statement_head(tokens, idx);
                ...
                if upper == "OJ"
                    ...
                    && matches!(
                        ...
                        Some((SqlToken::Symbol(symbol), _)) if symbol == "{"
                    )
                {
                    // ODBC outer-join escape: `{ OJ table ... }`. `OJ` is a
                    // grammar introducer, not the relation name or its alias.
                    // Keep the pending FROM target so the real first relation
                    // and every following JOIN retain ordinary scope handling.
                    last_word = None;
                    idx += 1;
                    continue;
                }
                ...
                match upper.as_str() {
                    "INSERT" => {
                        ...
                        if is_merge_action_keyword {
                            ...
                            depth_frames[depth].phase = SqlPhase::SetClause;
                            relation_state.clear();
                        } else if is_expression_context && !set_statement_embedded_head {
                    ...
                    "REPLACE" => {
                        ...
                        if (is_expression_context && !set_statement_embedded_head)
                    ...
                    "UPDATE" => {
                        ...
                        let is_locking_update_keyword = !set_statement_embedded_head
...
fn find_active_query_range(
    ...
    if let Some(range) = set_statement_embedded_query_range(tokens) {
        consider(range);
    }
...
fn set_statement_embedded_query_range(tokens: &[SqlToken]) -> Option<TokenRange> {
    let mut depth = 0usize;
    ...
    for (idx, token) in tokens.iter().enumerate() {
        match token {
            ...
            SqlToken::Word(word) if depth == 0 => {
                top_level_words.push((idx, word.to_ascii_uppercase()));
            }
            _ => {}
        }
    }
    if !matches!(
        top_level_words.as_slice(),
        [(_, set), (_, statement), ..] if set == "SET" && statement == "STATEMENT"
    ) {
        return None;
    }
    top_level_words.windows(2).find_map(|window| {
        let [(for_idx, for_word), (head_idx, head)] = window else {
            return None;
        };
        (for_word == "FOR"
            && matches!(
        ...
        .then_some(TokenRange {
            start: (*head_idx).max(for_idx.saturating_add(1)),
            end: tokens.len(),
        })
    })
}
...
fn parse_alias_deep_with_columns(
    tokens: &[SqlToken],
    start: usize,
) -> (Option<String>, usize, Vec<String>, Option<TokenRange>, bool) {
    let (start, has_oracle_flashback_versions) = scan_relation_postfix_clauses(tokens, start);
...
fn skip_relation_postfix_clauses(tokens: &[SqlToken], start: usize) -> usize {
    scan_relation_postfix_clauses(tokens, start).0
}

fn scan_relation_postfix_clauses(tokens: &[SqlToken], start: usize) -> (usize, bool) {
    let mut idx = skip_comment_tokens(tokens, start);
    let mut has_oracle_flashback_versions = false;
    ...
    loop {
        ...
        match upper.as_str() {
            ...
            "VERSIONS" => {
                ...
                idx = skip_comment_tokens(tokens, idx);
                has_oracle_flashback_versions = true;
                continue;
            }
            "AS" => {
    ...
    (idx, has_oracle_flashback_versions)
}
...
fn resolve_qualifier_table_refs<'a>(
    qualifier: &str,
    tables_in_scope: &'a [ScopedTableRef],
) -> Vec<&'a ScopedTableRef> {
    fn unique_deepest_match(candidates: Vec<&ScopedTableRef>) -> Vec<&ScopedTableRef> {
        let Some(max_depth) = candidates.iter().map(|table_ref| table_ref.depth).max() else {
            return Vec::new();
        };
        let mut deepest = candidates
            .into_iter()
            .filter(|table_ref| table_ref.depth == max_depth)
            .collect::<Vec<_>>();
        deepest.sort_by_key(|table_ref| normalize_identifier_for_lookup(&table_ref.name));
        deepest.dedup_by(|left, right| {
            normalize_identifier_for_lookup(&left.name)
                == normalize_identifier_for_lookup(&right.name)
        });
        if deepest.len() == 1 {
            deepest
        } else {
    ...
    for table_ref in tables_in_scope {
        ...
        if alias_upper.as_deref() == Some(qualifier_upper.as_str()) {
            alias_matches.push(table_ref);
            continue;
        }

        if name_upper == qualifier_upper {
            name_matches.push(table_ref);
            continue;
        }
        ...
        {
            short_name_matches.push(table_ref);
        }
    }
...
pub fn resolve_qualifier_tables(
    ...
    resolve_qualifier_table_refs(qualifier, tables_in_scope)
        .into_iter()
...
pub(crate) fn extract_select_list_columns(tokens: &[SqlToken]) -> Vec<String> {
    ...
    let projection_query_tokens = first_parenthesized_set_projection_query(tokens);
    let projection_query_tokens = first_top_level_set_projection_branch(projection_query_tokens);
    let select_list_tokens = extract_select_list_tokens(projection_query_tokens);
...
fn first_top_level_set_projection_branch(tokens: &[SqlToken]) -> &[SqlToken] {
    let depths = paren_depths(tokens);
    let end = tokens
        .iter()
        .enumerate()
        .find_map(|(idx, token)| {
            if !is_top_level_depth(&depths, idx) {
                return None;
            }
            let SqlToken::Word(word) = token else {
                return None;
            ...
            let mut next_idx = next_non_comment_index(tokens, idx + 1);
            if matches!(
                tokens.get(next_idx),
                Some(SqlToken::Word(modifier))
                    if modifier.eq_ignore_ascii_case("ALL")
                        || modifier.eq_ignore_ascii_case("DISTINCT")
            ) {
                next_idx = next_non_comment_index(tokens, next_idx + 1);
            }
            while matches!(tokens.get(next_idx), Some(SqlToken::Symbol(symbol)) if symbol == "(") {
                next_idx = next_non_comment_index(tokens, next_idx + 1);
            }
            matches!(
                tokens.get(next_idx),
                Some(SqlToken::Word(query))
                    if query.eq_ignore_ascii_case("SELECT")
                        || query.eq_ignore_ascii_case("WITH")
            )
            .then_some(idx)
        })
...
fn first_parenthesized_set_projection_query(mut tokens: &[SqlToken]) -> &[SqlToken] {
    for _ in 0..MAX_INTELLISENSE_RECURSION_DEPTH {
        let start = next_non_comment_index(tokens, 0);
        if start >= tokens.len() {
            return tokens;
        }
        let Some(SqlToken::Symbol(symbol)) = tokens.get(start) else {
            return tokens;
        };
        if symbol != "(" {
            return tokens;
        }
        let Some((range, next_idx)) = extract_parenthesized_range(tokens, start) else {
            return tokens;
        };
        let body = token_range_slice(tokens, range);
        if !is_query_expression_start(body, 0) {
            return tokens;
        }

        let after = next_non_comment_index(tokens, next_idx);
        if after < tokens.len()
            && !matches!(
                tokens.get(after),
                Some(SqlToken::Word(word))
...
/// Extract the projected aliases declared by Oracle `GRAPH_TABLE (... COLUMNS
/// (...))`. The graph pattern and row-shape clauses are internal to the table
/// function; only the COLUMNS items form the virtual relation's row type.
pub(crate) fn extract_oracle_graph_table_columns(tokens: &[SqlToken]) -> Vec<String> {
    let mut columns = Vec::new();
    let mut seen = HashSet::new();
    let mut graph_idx = 0usize;

    while graph_idx < tokens.len() {
        if !matches!(
            tokens.get(graph_idx),
            Some(SqlToken::Word(word)) if word.eq_ignore_ascii_case("GRAPH_TABLE")
        ) {
            graph_idx += 1;
            continue;
        }
        let open_idx = next_non_comment_index(tokens, graph_idx + 1);
        let Some((graph_range, next_idx)) = extract_parenthesized_range(tokens, open_idx) else {
            graph_idx += 1;
            continue;
        };
        let graph_tokens = token_range_slice(tokens, graph_range);
        let depths = paren_depths(graph_tokens);
        let Some(columns_idx) = graph_tokens.iter().enumerate().find_map(|(idx, token)| {
            (is_top_level_depth(&depths, idx)
                && matches!(
                    token,
                    SqlToken::Word(word) if word.eq_ignore_ascii_case("COLUMNS")
                ))
            .then_some(idx)
        }) else {
            graph_idx = next_idx;
        ...
        let columns_open_idx = next_non_comment_index(graph_tokens, columns_idx + 1);
        let Some((columns_range, _)) = extract_parenthesized_range(graph_tokens, columns_open_idx)
        else {
            graph_idx = next_idx;
            continue;
        };
        for item in
            split_top_level_symbol_groups(token_range_slice(graph_tokens, columns_range), ",")
        {
            if let Some(column) = resolve_item_column_name(&item) {
                let upper = normalize_identifier_for_scope_dedupe(&column);
                if seen.insert(upper) {
                    columns.push(column);
                }
            }
