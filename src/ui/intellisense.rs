...
/// Per-source suggestion budget. This intentionally exceeds the final popup
/// merge cap: completion applies context/type filters after reading a source, so
/// stopping at the display cap can leave fewer usable entries and hide later
/// exact matches.
pub(crate) const MAX_SUGGESTIONS: usize = 512;
...
pub struct IntellisenseData {
    ...
    /// Named table constraints addressable by `ALTER TABLE ... CONSTRAINT|CHECK`.
    ...
    constraint_entries: Vec<NameEntry>,
    ...
    /// Declared row object type of an Oracle object table or object view.
    object_type_by_relation: HashMap<String, String>,
...
impl IntellisenseData {
    pub fn new() -> Self {
        Self {
            ...
            object_type_by_relation: HashMap::new(),
    ...
    pub fn get_plsql_value_object_suggestions(&mut self, prefix: &str) -> Vec<String> {
        self.ensure_base_indices();
        Self::suggestions_from_entry_groups(
            prefix,
            &[
    ...
    pub fn get_constraint_object_suggestions(&mut self, prefix: &str) -> Vec<String> {
        self.ensure_base_indices();
        Self::suggestions_from_entry_groups(prefix, &[&self.constraint_entries])
    }
    ...
    pub fn set_object_type_for_relation(&mut self, relation: &str, type_name: &str) {
        let relation_key = Self::normalize_qualifier_lookup_key(relation);
        let type_key = Self::normalize_qualifier_lookup_key(type_name);
        if relation_key.is_empty() || type_key.is_empty() {
            return;
        }
        self.object_type_by_relation.insert(relation_key, type_key);
    }

    pub fn object_type_for_relation(&self, relation: &str) -> Option<&str> {
        for key in Self::qualifier_lookup_keys(relation) {
            if let Some(type_name) = self.object_type_by_relation.get(&key) {
                return Some(type_name.as_str());
            }
        }
        None
    ...
    pub fn rebuild_indices(&mut self) {
        ...
        self.constraint_entries = Self::build_entries(&self.constraints);
    ...
    fn ensure_base_indices(&mut self) {
        if self.table_entries.len() != self.tables.len()
            ...
            || self.constraint_entries.len() != self.constraints.len()
    ...
    fn push_matching_entries(
        ...
        let typed_delimiter = identifier_quote_delimiter(raw_prefix);
        let allow_unquoted_candidate_for_quoted_prefix = typed_delimiter.is_some()
            && !raw_prefix.is_empty()
            && !entries
                .iter()
                .skip(start)
        ...
        for entry in entries.iter().skip(start) {
            ...
            let entry_lookup = strip_matching_identifier_quotes(&entry.name);
            let entry_lookup_upper = entry_lookup.to_ascii_uppercase();
            let keyword_requires_quotes = sql_text::is_mysql_sql_keyword(&entry_lookup_upper)
                || sql_text::is_oracle_sql_keyword(&entry_lookup_upper);
            let rendered = typed_delimiter
                .filter(|delimiter| {
                    keyword_requires_quotes
                        && identifier_quote_delimiter(&entry.name) != Some(*delimiter)
                })
                .map_or_else(
                    || entry.name.clone(),
                    |delimiter| render_identifier_with_delimiter(&entry.name, delimiter),
                );
            let unquoted_candidate_matches_quoted_prefix =
                allow_unquoted_candidate_for_quoted_prefix
                    && identifier_quote_delimiter(&entry.name).is_none();
            if !raw_prefix.is_empty()
                && !suggestion_matches_completion_prefix(&rendered, raw_prefix)
                && !unquoted_candidate_matches_quoted_prefix
            {
                continue;
            }
...
fn render_identifier_with_delimiter(value: &str, opening: char) -> String {
    let value = strip_matching_identifier_quotes(value);
    let (closing, escaped) = match opening {
        '"' => ('"', value.replace('"', "\"\"")),
        '`' => ('`', value.replace('`', "``")),
        '[' => (']', value.replace(']', "]]")),
        _ => return value.into_owned(),
    };
    format!("{opening}{escaped}{closing}")
}
...
pub(crate) fn get_word_at_cursor_with_lexical_mode(
    ...
    {
        if line_has_unclosed_identifier_delimiter(text, effective_pos, delimiter, &lexical_spans) {
    ...
    let (word, start, end) = get_unquoted_word_at_cursor(text, effective_pos);
    if let Some((quote_start, delimiter)) = text
        .get(..start)
        .and_then(|head| head.char_indices().next_back())
        .filter(|(_, ch)| (*ch == '"' && !mysql_compatible) || (*ch == '`' && mysql_compatible))
        .filter(|(quote_start, _)| quoted_identifier_start_context(text, *quote_start))
    {
        let end = quoted_identifier_end_from_cursor(text, effective_pos, delimiter).unwrap_or(end);
        return (
            text.get(quote_start..effective_pos)
                .unwrap_or("")
                .to_string(),
            quote_start,
            end,
...
fn line_has_unclosed_identifier_delimiter(
    text: &str,
    cursor_pos: usize,
    delimiter: char,
    lexical_spans: &[crate::sql_parser_engine::LexicalSpan],
    ...
    let line_start = text
        ...
        .map_or(0, |idx| idx + 1);
    let count = text
        .get(line_start..cursor_pos)
        .unwrap_or("")
        .char_indices()
        .filter(|(_, ch)| *ch == delimiter)
        .filter(|(relative_idx, _)| {
            let idx = line_start + relative_idx;
            let span_idx = lexical_spans.partition_point(|span| span.end <= idx);
            lexical_spans
                .get(span_idx)
                .filter(|span| span.contains(idx))
                .is_none_or(|span| {
                    span.kind == crate::sql_parser_engine::LexicalKind::QuotedIdentifier
                })
        })
        .count();
    crate::utils::arithmetic::safe_rem(count, 2) == 1
}
...
BEGIN
  pkg.proc(p_)"#;
    ...
    #[test]
    fn get_word_at_cursor_recovers_adjacent_quote_after_stale_bounded_mode() {
        let text = r#"bounded fragment rec."rows" FROM dual"#;
        let cursor = text.find("s\" FROM").expect("expected cursor anchor");
        let (word, start, end) = get_word_at_cursor_with_lexical_mode(
            text,
            cursor,
            false,
        ...
        assert_eq!(word, r#""row"#);
        assert_eq!(text.get(start..cursor), Some(r#""row"#));
        assert_eq!(text.get(start..end), Some(r#""rows""#));
    }

    #[test]
    fn get_word_at_cursor_rejects_a_completed_backtick_as_a_stale_opener() {
        let text = "a.`--` + a.`/*`";
        let cursor = text.find(".`/*`").expect("expected cursor anchor");
        let (word, start, _) = get_word_at_cursor_with_lexical_mode(
            text,
            cursor,
            true,
        ...
        assert_eq!(word, "a");
        assert_eq!(text.get(start..cursor), Some("a"));
    }
    ...
    #[test]
    fn get_column_suggestions_render_with_the_typed_quote_delimiter() {
        let mut data = IntellisenseData::new();
        data.set_virtual_table_columns("REC", vec![r#""select""#.to_string()]);

        let scope = vec!["REC".to_string()];
        assert_eq!(
            data.get_column_suggestions("`sele", Some(scope.as_slice())),
