...
impl PendingTailTokenKind {
    fn from_states(
        ...
        match lex_mode {
            ...
            | LexMode::ForeignInlineSource { .. } => Self::String,
            LexMode::DoubleQuote
...
fn span_before_alias_is_relation_reference(spans: &[SqlTokenSpan], prev_idx: usize) -> bool {
    ...
    let relation_is_qualified = previous_significant_idx(spans, prev_idx)
        .is_some_and(|idx| span_is_symbol(spans.get(idx), "."));
    if !relation_is_qualified
        && span_word_upper(spans.get(prev_idx)).is_some_and(|word| {
            word != "DUAL"
    ...
    F: FnMut() -> bool,
{
    ...
    // An MLE unit's body is JavaScript. It is emitted as a single opaque token so
    // no SQL rule - comment, quote, operator spacing - is applied inside it.
    let is_mle_module_statement = sql_text::starts_create_mle_module(sql);
    let carries_foreign_source = sql_text::carries_foreign_language_source(sql);
    ...
    while let Some((idx, c)) = iter.next() {
        ...
        if let crate::sql_parser_engine::LexMode::ForeignInlineSource { brace_depth } =
        ...
        if matches!(scan_state.lex_mode, crate::sql_parser_engine::LexMode::Idle)
            && current.is_empty()
        {
            // `CREATE MLE MODULE ... AS` hands the rest of the statement to
            // another language; the inline call-spec form opens with `{{`.
            let follows_module_as = is_mle_module_statement
                && matches!(
                    tokens.last().map(|span| &span.token),
                    Some(SqlToken::Word(word)) if word.eq_ignore_ascii_case("AS")
                );
            if follows_module_as {
                current_start = idx;
                current.push(c);
                scan_state.lex_mode = crate::sql_parser_engine::LexMode::ForeignModuleSource;
            ...
            if carries_foreign_source && c == '{' && next == Some('{') {
                current_start = idx;
                current.push(c);
                scan_state.lex_mode =
                    crate::sql_parser_engine::LexMode::ForeignInlineSource { brace_depth: 1 };
                continue;
            }
        }
...
END$$"#;
    ...
    #[test]
    fn tokenize_sql_treats_oracle_exponentiation_as_single_symbol() {
        let tokens = tokenize_sql_with_mysql_compat("powered := base ** 10", false);

        assert!(
            tokens
                .iter()
                .any(|token| matches!(token, SqlToken::Symbol(symbol) if symbol == "**")),
            "Oracle exponentiation must stay a single symbol token: {tokens:?}"
        );
    }

    #[test]
    fn tokenize_sql_treats_oracle_caret_not_equal_as_single_symbol() {
        let sql = "SELECT 1 ^= 2 FROM dual";

        for tokens in [
            tokenize_sql_with_mysql_compat(sql, false),
            tokenize_sql_reference(sql),
        ] {
            assert!(
                tokens
                    .iter()
                    .any(|token| matches!(token, SqlToken::Symbol(symbol) if symbol == "^=")),
                "Oracle caret not-equal must stay a single symbol token: {tokens:?}"
            );
        }
    }
    ...
    #[test]
    fn local_alias_context_collects_dual_alias_after_comma() {
        let sql = "SELECT f.value, d.dummy FROM facts f, dual d";
        let aliases = collect_local_alias_context(sql);
        let d_start = sql.rfind("dual d").unwrap() + "dual ".len();

        assert!(aliases.contains_name("d"));
        assert!(aliases.is_declaration_range(d_start, d_start + 1));
    }

    #[test]
    fn wave11_local_alias_context_collects_qualified_keyword_relation_aliases() {
        let sql =
            "SELECT c.COLUMN_NAME FROM information_schema.COLUMNS c JOIN mysql.user u ON 1 = 1";
        let aliases = collect_local_alias_context(sql);

        for alias in ["c", "u"] {
            let start = sql
                .rfind(&format!(" {alias}"))
                .expect("qualified relation alias")
                + 1;
            assert!(aliases.contains_name(alias), "missing alias `{alias}`");
            assert!(aliases.is_declaration_range(start, start + alias.len()));
        }
    }
