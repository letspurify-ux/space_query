...
impl SqlEditorWidget {
    ...
    fn bounded_intellisense_parse_text_from_chunked_with_window(
        ...
        let end = text.clamp_boundary(cursor_pos.saturating_add(lookahead_bytes).min(text.len()));
        BoundedIntellisenseParseText {
            text: text
    ...
    fn expanded_statement_window_in_bounded_text_for_db_type(
        ...
        } else if completion_db_type_is_mariadb(preferred_db_type) {
            if let Some(enclosing) =
                Self::mariadb_package_enclosing_window_in_bounded_text(bounded, &expanded)
            {
                expanded = enclosing;
            }
        }
    ...
    fn mariadb_package_enclosing_window_in_bounded_text(
        bounded: &BoundedIntellisenseParseText,
        ...
        let meaningful = spans
            .iter()
            .enumerate()
            .filter(|(_, span)| span.start < cursor && !matches!(span.token, SqlToken::Comment(_)))
            .collect::<Vec<_>>();
        let mut root_start = None;
        for window_start in 0..meaningful.len() {
            let (_, create) = meaningful[window_start];
            if !Self::token_word(&create.token)
                .is_some_and(|word| word.eq_ignore_ascii_case("CREATE"))
            {
                continue;
            }
            let mut idx = window_start + 1;
            if meaningful
                .get(idx)
                .and_then(|(_, span)| Self::token_word(&span.token))
            ...
            if meaningful
                .get(idx)
                .and_then(|(_, span)| Self::token_word(&span.token))
                .is_some_and(|word| word.eq_ignore_ascii_case("PACKAGE"))
                && meaningful
                    .get(idx + 1)
                    .and_then(|(_, span)| Self::token_word(&span.token))
                    .is_some_and(|word| word.eq_ignore_ascii_case("BODY"))
            {
                root_start = Some(create.start);
            }
        }
        ...
        let delimiter = text
            ...
            .filter_map(|line| {
                ...
                parts
                    ...
                    .then(|| parts.next().unwrap_or(";").to_string())
            })
            .next_back()
            .unwrap_or_else(|| ";".to_string());
        let region_end = if delimiter == ";" {
            text.len()
        } else {
            let tail = text.get(root_start..).unwrap_or("");
            root_start.saturating_add(tail.find(&delimiter)?)
        };
    ...
    fn oracle_plsql_enclosing_window_in_bounded_text(
        ...
        if text.get(..cursor).is_some_and(|before_cursor| {
        ...
        let offset_is_literal_or_comment = |candidate_offset: usize| {
            let idx =
                literal_or_comment_ranges.partition_point(|(_, end)| *end <= candidate_offset);
            literal_or_comment_ranges
                .get(idx)
                .is_some_and(|(start, end)| *start <= candidate_offset && candidate_offset < *end)
        };

        for line_with_newline in text.split_inclusive('\n') {
            let line_end = offset
                .saturating_add(line_with_newline.len())
                .min(text.len());
            ...
            let line_starts_in_literal_or_comment = offset_is_literal_or_comment(candidate_start);
    ...
    fn statement_window_from_bounded_text_bounds(
        ...
        let statement_start = Self::statement_start_with_leading_plsql_label(text, statement_start);
        ...
        let statement_text = text
            ...
            .unwrap_or("")
            .to_string();
        let absolute_statement_start = text_start.saturating_add(statement_start);
    ...
    fn trim_expanded_statement_leading_block_comment_fragment_in_bounded_text(
        text: &str,
        text_start: usize,
        cursor_pos: usize,
        expanded: ExpandedStatementWindow,
        ...
        let statement_start = expanded.statement_start.saturating_sub(text_start);
        let statement_end = expanded.statement_end.saturating_sub(text_start);
        let mut new_start = statement_start
            .saturating_add(close_idx)
            .saturating_add(2)
            .min(statement_end);
        while new_start < statement_end {
            let Some(rest) = text.get(new_start..statement_end) else {
                break;
            };
        ...
        if text_start.saturating_add(new_start) >= cursor_pos || new_start >= statement_end {
            return expanded;
        }
        Self::statement_window_from_bounded_text_bounds(
            text,
    ...
    fn statement_start_with_leading_plsql_label(text: &str, statement_start: usize) -> usize {
        ...
        let inner = label_text
            ...
            .map(str::trim)
            .unwrap_or("");
        if inner.is_empty() || !inner.chars().all(crate::sql_text::is_identifier_char) {
            return statement_start;
        }
        label_start
    }
    ...
    fn expanded_statement_window_and_text_binds_from_cursor_analysis_cancellable(
        ...
        let (expanded, text_bind_names, package_spec_symbols) =
            Self::expanded_statement_window_and_text_binds_from_bounded(preferred_db_type, bounded);
        if cancellation.is_some_and(IntellisenseCancellation::is_cancelled) {
            return None;
        }
    ...
    fn build_routine_symbol_cache_entry_with_token_spans(
        ...
        package_spec_symbols: &[ParsedDeclarationSymbol],
        preferred_db_type: Option<crate::db::connection::DatabaseType>,
        shared_sql_context: Option<(Vec<SqlTokenSpan>, super::query_text::LocalAliasContext)>,
    ) -> RoutineSymbolCacheEntry {
    ...
    fn build_routine_symbol_cache_entry_with_token_spans_cancellable(
        ...
        for (idx, span) in token_spans.into_iter().enumerate() {
            if idx & 0xff == 0 && cancellation.is_some_and(IntellisenseCancellation::is_cancelled) {
                return None;
            }
    ...
    fn collect_local_symbol_suggestions_for_db(
        ...
        if Self::cursor_is_at_mysql_end_label_slot_for_context(
            ...
            return Self::mysql_expected_end_label_suggestions(analysis.context.as_ref(), prefix);
        }
        ...
        for symbol in analysis.local_symbols.iter() {
            ...
            let Some(scope_rank) = scope_rank_by_id.get(symbol.scope_id).and_then(|rank| *rank)
            else {
                continue;
            };
        ...
        if let Some(exception_names) = exception_name_filter {
            suggestions.retain(|name| exception_names.contains(&name.to_ascii_uppercase()));
            return suggestions;
        }
    ...
    fn collect_bounded_oracle_routine_symbol_suggestions(
        ...
        let cache = Self::build_routine_symbol_cache_entry(0, &expanded, Vec::new(), &[], db_type);
        let analysis =
            Self::build_intellisense_analysis_from_routine_cache(&cache, scan_text.len());
        Self::collect_local_symbol_suggestions_for_db(
            prefix,
            scan_text.len(),
            &analysis,
    ...
    fn collect_plsql_named_scope_member_suggestions(
        qualifier: &str,
        prefix: &str,
        cursor_in_statement: usize,
        statement_text: &str,
        ...
        let cursor_in_statement = cursor_in_statement.min(statement_text.len());
        let active_scope = Self::deepest_local_scope_at_cursor(
            analysis.local_scopes.as_ref(),
            cursor_in_statement,
        );
        ...
        let label_marker = format!("<<{qualifier_upper}>>");
        let statement_upper = statement_text
            .get(..cursor_in_statement)
            .unwrap_or(statement_text)
            .to_ascii_uppercase();
        let label_end = statement_upper
            .rfind(&label_marker)
            .map(|start| start.saturating_add(label_marker.len()));
        let label_scope = label_end.and_then(|label_end| {
            scope_chain
                .iter()
                .copied()
                .filter(|scope_id| {
                    analysis
                        .local_scopes
                        .get(*scope_id)
                        .is_some_and(|scope| scope.start >= label_end)
                })
                .min_by_key(|scope_id| {
                    analysis
                        .local_scopes
                        .get(*scope_id)
                        .map_or(usize::MAX, |scope| scope.start)
                })
        });

        let routine_scope = if label_scope.is_none() {
            let tokens = analysis.context.statement_tokens.as_ref();
            let routine_name = tokens.iter().enumerate().find_map(|(idx, token)| {
                ...
                Self::token_word(tokens.get(name_idx)?)
                    .map(Self::local_identifier_lookup_upper)
            });
            (routine_name.as_deref() == Some(qualifier_upper.as_str())).then(|| {
                scope_chain.iter().copied().find(|scope_id| {
                    analysis
                        .local_scopes
                        .get(*scope_id)
                        .is_some_and(|scope| matches!(scope.kind, LocalScopeKind::Routine))
                })
            })?
        } else {
            None
        };
        let scope_id = label_scope.or(routine_scope)?;
        let prefix_upper = Self::local_member_suggestion_lookup_upper(prefix);
        let mut suggestions = Vec::new();
        let mut seen = HashSet::new();
        for symbol in analysis.local_symbols.iter() {
            if symbol.scope_id != scope_id
                || symbol.declared_at > cursor_in_statement
                || !symbol.suggest_name
                || !Self::local_symbol_matches_prefix(
                    &symbol.name,
                    &symbol.upper,
                    &prefix_upper,
                )
    ...
    fn collect_local_qualified_expression_field_suggestions(
        ...
        let symbol =
            Self::visible_local_symbol_for_qualifier(type_name, cursor_in_statement, analysis)?;
    ...
    fn is_builtin_collection_object_type(source_upper: &str) -> bool {
        ...
        matches!(
            unqualified,
            "JSON_KEY_LIST" | "ODCINUMBERLIST" | "ODCIVARCHAR2LIST"
        )
    }
    ...
    fn current_local_assignment_expected_operand_type(
        ...
        match operand_type {
            ...
            | PrecedingOperandType::Collection => Some(ExpectedOperandTypes::Single(operand_type)),
    ...
    fn current_local_into_target_expected_operand_type(
        ...
        let projection_index = Self::current_select_list_anchor_before_cursor(tokens, end)
            .and_then(|select_idx| {
                let mut depth = 0i32;
                for token in tokens
                    .get(select_idx + 1..end.min(tokens.len()))
                    .unwrap_or(&[])
                {
                    match token {
                ...
                (depth == 0)
                    .then(|| Self::current_select_projection_index(tokens, select_idx, end))
            })
            .or_else(|| {
                Self::current_returning_list_anchor_before_cursor(tokens, end).map(
                    |returning_idx| {
    ...
    fn current_returning_list_anchor_before_cursor(
        tokens: &[SqlToken],
        end: usize,
    ) -> Option<usize> {
        let mut depth = 0i32;
    ...
    fn resolve_local_member_scope_for_qualifier(
        ...
        if segment_indexed.first().copied().unwrap_or(false)
    ...
    fn normalize_raw_qualifier_segment(raw_segment: &str) -> Option<String> {
        ...
        let (segment, segment_start) =
            Self::parse_qualifier_segment_before_dot(raw_segment, segment_end, &lexical_kinds)?;
        if segment_start != 0 {
            return None;
        }
        Some(segment)
    ...
    fn resolve_local_member_entry_scope(
        ...
        if let Some(members) = Self::builtin_object_type_members(source_upper) {
            return Some(ResolvedLocalMemberScope {
                ...
                member_entries: Vec::new(),
                member_source_upper: Some(source_upper.to_string()),
                member_source_uppers: Vec::new(),
                member_source_is_rowtype: false,
                member_source_is_collection_like: Self::is_builtin_collection_object_type(
                    source_upper,
                ),
            });
        }
        ...
        while idx < token_spans.len() {
            ...
            match &token.token {
                ...
                SqlToken::Word(word) => {
                    ...
                    if upper == "PACKAGE" && root_decl_start_idx.is_none() && !prev_is_end {
                    ...
                    if matches!(upper.as_str(), "PROCEDURE" | "FUNCTION") && !prev_is_end {
                    ...
                    match upper.as_str() {
                        ...
                        "REPEAT" => {
                            if !Self::next_meaningful_token_idx(token_spans, idx + 1).is_some_and(
                                |next_idx| Self::token_symbol_at(token_spans, next_idx, "("),
                            ) {
                                block_stack.push(LocalBlockFrame {
    ...
    fn extract_declaration_symbol_from_item(
        item: &[SqlTokenSpan],
        ...
        if is_function {
            ...
            while idx < item.len() {
                match &item[idx].token {
                    ...
                    SqlToken::Symbol(sym) if sym == "(" => depth = depth.saturating_add(1),
                    SqlToken::Symbol(sym) if sym == ")" => depth = depth.saturating_sub(1),
                    SqlToken::Word(word) if depth == 0 && word.eq_ignore_ascii_case("RETURN") => {
                        return_type_display = Self::next_meaningful_token_idx(item, idx + 1)
                            .and_then(|type_idx| Self::scalar_type_display_at_idx(item, type_idx));
                        break;
                    }
                    _ => {}
                }
    ...
    fn extract_record_type_fields(
        ...
        while idx <= close_idx {
            ...
            if is_boundary {
                if let Some(entry) =
                    Self::record_field_entry_from_item_range(item, field_start, idx)
                {
    ...
    fn record_field_entry_from_item_range(
        ...
        // A record field occupies an unambiguous declaration slot, so contextual
        // SQL/PLSQL words such as `TYPE` are legal field names here. The broader
        // declaration parser still rejects those words where they would denote a
        // declaration kind rather than a name.
        let normalized_name = Self::local_identifier_from_word_with_keyword_policy(raw_name, true)?;
        ...
        let (type_display, member_source_upper, member_source_is_rowtype) = if let Some(type_idx) =
            Self::next_meaningful_token_idx(item, name_idx + 1).filter(|idx| *idx < end_idx)
        {
                    ...
                    .filter(|(_, source_end_idx)| *source_end_idx < end_idx)
            {
                ...
                (
                    type_display,
                    Some(source_name.to_ascii_uppercase()),
                    is_rowtype,
                )
            } else {
    ...
    fn extract_mysql_declaration_symbols_from_item(
        ...
        if let Some(row_idx) = kind_idx.filter(|idx| {
            Self::token_word(&item[*idx].token).is_some_and(|word| word.eq_ignore_ascii_case("ROW"))
        }) {
            ...
            if type_idx.is_some_and(|idx| {
                ...
                if let Some((source, _)) =
                    source_idx.and_then(|idx| Self::extract_declaration_type_source_name(item, idx))
                {
        ...
        while item_end < limit {
            match &token_spans[item_end].token {
                ...
                SqlToken::Word(word) if paren_depth == 0 => {
                    let upper = word.to_ascii_uppercase();
                    let previous_was_end = previous_word_upper
                        .as_deref()
                        .is_some_and(|prev| prev == "END");
                    match upper.as_str() {
                        "BEGIN" | "REPEAT" => {
    ...
    fn parse_for_loop_record(tokens: &[SqlTokenSpan], idx: usize) -> Option<ParsedForLoopRecord> {
        ...
        let name = Self::token_word(&tokens[name_idx].token)
            .and_then(Self::local_identifier_suggestion_from_word)?;
    ...
    fn resolve_deferred_local_record_members(
        ...
        for pass in 0..symbols.len() {
            if pass & 0x0f == 0 && cancellation.is_some_and(IntellisenseCancellation::is_cancelled)
            {
                return false;
            }
            ...
            for (idx, symbol) in symbols.iter().enumerate() {
                ...
                for &candidate_idx in candidate_indices {
                    ...
                    if best.is_none_or(|(best_rank, best_declared_at, _)| {
                        scope_rank < best_rank
                            || (scope_rank == best_rank
                                && candidate.declared_at >= best_declared_at)
                    }) {
                ...
                if let Some((_, _, candidate_idx)) = best {
                    ...
                    if !candidate.members.is_empty() {
                        ...
                        resolved_rowtype_sources[idx] =
                            Some(candidate.member_source_uppers.clone());
            ...
            for (
                (
                    ((((symbol, members), member_entries), rowtype_metadata), rowtype_sources),
                    collection_like,
                ),
    ...
    fn extract_select_projection_members_and_source(
        ...
        let mut wildcard_tables = intellisense_context::extract_select_list_wildcard_scopes(
            body_tokens,
            &tables_in_scope,
        );
    ...
    fn extract_virtual_wildcard_projection_members(
        ...
        for subquery in &ctx.subqueries {
            let mut members = if subquery.explicit_columns.is_empty() {
                Self::extract_known_virtual_projection_members(
                    intellisense_context::token_range_slice(
                        ctx.statement_tokens.as_ref(),
                        subquery.body_range,
                    ),
            ...
            let key = ctx
                ...
                .find_map(|table_ref| {
                    ...
                    (table_ref.name.eq_ignore_ascii_case(&table)
                        && virtual_members_by_name.contains_key(&alias.to_ascii_uppercase()))
    ...
    fn extract_known_virtual_projection_members(
        body_tokens: &[SqlToken],
    ) -> VirtualProjectionMembers {
    ...
    fn extract_package_spec_symbols(
        ...
        while cursor < spans.len() {
            match &spans[cursor].token {
                ...
                SqlToken::Symbol(sym) if sym == ")" => paren_depth = paren_depth.saturating_sub(1),
                SqlToken::Word(word) if paren_depth == 0 => {
                    let upper = word.to_ascii_uppercase();
                    match upper.as_str() {
    ...
    fn package_spec_candidate_matches(
        ...
        loop {
            ...
            let Some((seg_start, seg_end)) = Self::next_identifier_word_range(bytes, dot + 1)
            else {
                break;
            };
    ...
    fn collect_declaration_items_symbols(
        ...
        while idx < end {
            ...
            if let Some(symbol) =
                Self::extract_declaration_symbol_from_item(&spans[item_start..item_end])
            {
                symbols.push(symbol);
            }
            idx = item_end;
    ...
    fn parse_routine_header(tokens: &[SqlTokenSpan], idx: usize) -> Option<ParsedRoutineHeader> {
        ...
        let mut scan_idx =
            Self::next_meaningful_token_idx(tokens, name_idx + 1).unwrap_or(tokens.len());
        ...
        if scan_idx < tokens.len() && Self::token_symbol_at(tokens, scan_idx, "(") {
            ...
            scan_idx = Self::next_meaningful_token_idx(tokens, close_idx.saturating_add(1))
        ...
        while scan_idx < tokens.len() {
            match &tokens[scan_idx].token {
                ...
                SqlToken::Word(word)
                    if paren_depth == 0 && word.eq_ignore_ascii_case("RETURNS") =>
                {
    ...
    fn extract_parameter_symbol_from_item(
        item: &[SqlTokenSpan],
    ) -> Option<ParsedDeclarationSymbol> {
    ...
    fn collect_text_bind_names_before_statement(
        ...
        let mut names =
            Self::collect_text_var_bind_names_before_statement(full_text, statement_start);
    ...
    fn build_routine_symbol_cache_bundle_for_test(
        ...
        Self::build_routine_symbol_cache_bundle_for_test_for_db_type(full_text, cursor_pos, None)
    }
    ...
    fn collect_local_symbol_suggestions_with_prefix_for_test(
        ...
        Self::collect_local_symbol_suggestions_with_prefix_for_db_type_for_test(
            script_with_cursor,
            prefix,
            session_bind_names,
            None,
    ...
    #[cfg(test)]
    fn collect_local_symbol_suggestions_with_prefix_for_db_type_for_test(
        script_with_cursor: &str,
        prefix: &str,
        session_bind_names: &[&str],
        db_type: Option<crate::db::connection::DatabaseType>,
        ...
        let (routine_cache, expanded) =
            Self::build_routine_symbol_cache_bundle_for_test_for_db_type(&sql, cursor, db_type);
    ...
    fn collect_local_member_suggestions_for_test(
        ...
        let local_record_member_suggestions = local_record_member_suggestions.unwrap_or_default();
