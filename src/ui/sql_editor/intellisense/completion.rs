...
struct ExpressionKeywordContext {
    ...
    /// Whether the cursor is in MySQL's top-level `DO` expression list. The
    /// slot is a value expression, not a statement/object position, so statement
    /// heads and relation names from the flat base catalog are noise.
    in_mysql_do_value_expression: bool,
    /// Whether the cursor is in MySQL's top-level `HELP` topic argument. The
    /// topic text is not a SQL expression/object slot, so the SQL catalog must
    /// stay silent after the command head.
    in_mysql_help_topic_argument: bool,
    /// Whether the cursor is in a value expression of MySQL's top-level `SET`
    /// assignment syntax (`SET @v = |`, `SET sql_mode = |`). This overlaps with
    /// SQL*Plus `SET`, so it must be recognized as SQL only in MySQL mode.
    in_mysql_set_assignment_value_expression: bool,
    /// Whether the cursor is in MySQL `EXPLAIN ... FOR SCHEMA|DATABASE |`.
    /// The name is a free schema/database identifier, not a table/object/value
    /// slot, so ordinary SQL catalog suggestions must stay silent there.
    at_mysql_explain_schema_name: bool,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum MysqlExplainSchemaSlot {
    SchemaName,
    AfterSchemaName { analyze: bool },
}
...
impl ExpressionKeywordContext {
    ...
    fn ambiguous() -> Self {
        Self {
            ...
            in_mysql_do_value_expression: false,
            in_mysql_help_topic_argument: false,
            in_mysql_set_assignment_value_expression: false,
            at_mysql_explain_schema_name: false,
...
impl SqlEditorWidget {
    ...
    fn apply_intellisense_with_context(
        ...
        let effective_expected_operand_type = expr_keyword_ctx
            ...
            .or_else(|| {
                Self::current_declaration_default_expected_operand_type(
                    deep_ctx.statement_tokens.as_ref(),
                    deep_ctx.cursor_token_len,
                )
            })
    ...
    fn expression_keyword_context(
        ...
        let mut follows_operand = Self::cursor_follows_complete_operand(tokens, end);
        ...
        if negated_predicate_operator_not_idx.is_none()
            && matches!(
        ...
        let in_mysql_do_value_expression =
            crate::sql_text::mysql_compatibility_for_sql("", db_type)
                && Self::cursor_has_statement_start_anchor(full_tokens, full_end, &["DO"]);
        let in_mysql_help_topic_argument =
            crate::sql_text::mysql_compatibility_for_sql("", db_type)
                && Self::cursor_has_statement_start_anchor(full_tokens, full_end, &["HELP"]);
        let in_mysql_set_assignment_value_expression =
            crate::sql_text::mysql_compatibility_for_sql("", db_type)
                && Self::cursor_in_mysql_set_assignment_value(full_tokens, full_end);
        let at_mysql_explain_schema_name =
            crate::sql_text::mysql_compatibility_for_sql("", db_type)
                && matches!(
                    Self::mysql_explain_schema_slot(full_tokens, full_end),
        ...
        ExpressionKeywordContext {
            ...
            in_mysql_do_value_expression,
            in_mysql_help_topic_argument,
            in_mysql_set_assignment_value_expression,
            at_mysql_explain_schema_name,
            prev_operand_type,
            expected_operand_type,
    ...
    fn expected_operand_type(
        ...
        if Self::current_pattern_lhs_expected_type(tokens, end, mysql_compatible) {
            return Some(ExpectedOperandTypes::Single(PrecedingOperandType::Character));
        }
        if let Some(expected_type) =
            Self::current_comparison_lhs_expected_type(tokens, end, data, column_scope)
        {
            return Some(ExpectedOperandTypes::Single(expected_type));
        }
        if let Some(expected_type) =
            Self::current_is_distinct_from_lhs_expected_type(tokens, end, data, column_scope)
        {
            return Some(ExpectedOperandTypes::Single(expected_type));
        }
        if let Some(expected_type) =
    ...
    fn cursor_is_at_extract_source_operand(tokens: &[SqlToken], end: usize) -> bool {
        ...
        let Some(SqlToken::Word(function_name)) =
            Self::previous_non_comment_token_index(tokens, open_idx).and_then(|idx| tokens.get(idx))
        else {
            return false;
        };
    ...
    fn top_level_function_argument_bounds_after_cursor(
        ...
        let close_idx =
            Self::matching_close_paren_index_before(tokens, open_idx, tokens.len()).unwrap_or(tokens.len());
        if close_idx <= limit {
            return None;
        }
    ...
    fn current_case_result_expected_type(
        ...
        Self::next_case_result_range_after_cursor(tokens, active_result_start)
            .and_then(|(start, result_end)| {
                Self::expected_type_from_argument_range(tokens, start, result_end, data, column_scope)
            })
    }
    ...
    fn current_update_set_tuple_expected_type(
        ...
        let argument_index = Self::current_function_argument_index(tokens, value_list_open_idx, limit);
    ...
    fn current_pattern_lhs_expected_type(
        ...
        matches!(operator_upper.as_str(), "LIKE" | "LIKE2" | "LIKE4" | "LIKEC")
    ...
    fn expected_expression_construct_continuation_keywords(
        ...
        if let Some(frame) = stack.iter().next_back() {
            match frame {
                Frame::Between { .. } => {
    ...
    fn expected_row_limiting_keyword_candidates(
        ...
        const FETCH_DIRECTION_KEYWORDS: &[&str] = &["FIRST", "NEXT"];
        ...
        if len >= 1 && word(1) == Some("FETCH") {
            return Some(FETCH_DIRECTION_KEYWORDS);
        }
    ...
    fn expected_select_list_from_keyword_candidates(
        ...
        let has_top_level_from_after_cursor = tokens
            .iter()
            ...
            .any(|token| {
                match token {
                    ...
                    SqlToken::Word(word) if after_depth == 0 && word.eq_ignore_ascii_case("FROM") => {
                        true
                    }
                    _ => false,
                }
        ...
        follows_complete_projection.then_some(&["FROM"])
    }
    ...
    fn expected_dml_trailing_clause_keywords(
        ...
        match head {
            "UPDATE" if contains("SET") && !has_returning && complete_assignment_or_predicate => {
                if contains("WHERE") {
                    if mysql {
                        Some(&["ORDER BY", "LIMIT"])
                    } else {
            ...
            "DELETE" if contains("WHERE") && !has_returning && complete_assignment_or_predicate => {
                if mysql {
                    Some(&["ORDER BY", "LIMIT"])
                } else {
                    Some(&["RETURNING"])
    ...
    fn expected_plsql_and_routine_keyword_candidates(
        ...
        match words.as_slice() {
            ...
            [.., for_word, each] if for_word == "FOR" && each == "EACH" => {
                return Some(&["ROW"])
            }
            _ => {}
        }
        ...
        if first == Some("WITH") {
            ...
            if !mysql {
                match words.as_slice() {
                    ...
                    [.., cycle, _col, set, _mark]
                        if cycle == "CYCLE" && set == "SET" =>
                    {
                        return Some(&["TO"])
                    }
    ...
    fn expected_tool_command_supplement_keyword_candidates(
        ...
        match words.as_slice() {
            ...
            [show, errors] if show == "SHOW" && errors == "ERRORS" => {
                Some(&["PACKAGE", "PROCEDURE", "FUNCTION", "TRIGGER", "TYPE", "VIEW"])
            }
            [spool] if spool == "SPOOL" => Some(&["OFF", "OUT", "APPEND"]),
            [whenever] if whenever == "WHENEVER" => Some(&["SQLERROR", "OSERROR"]),
            [whenever, error] if whenever == "WHENEVER" && matches!(error.as_str(), "SQLERROR" | "OSERROR") => {
                Some(&["EXIT", "CONTINUE"])
            }
            [whenever, error, exit]
    ...
    fn expected_common_predicate_keyword_candidates(
        ...
        match expr_keyword_ctx.and_then(|ctx| ctx.follows_operand) {
            ...
            Some(false) if follows_comparison_operator => &[
                "ALL", "ANY", "SOME", "CASE", "CAST", "EXISTS", "NOT", "NULL", "TRUE", "FALSE",
                "UNKNOWN", "INTERVAL", "DATE", "TIMESTAMP", "TIME", "ROWNUM", "ROWID",
            ],
            Some(false) => &[
                "CASE",
                "CAST",
    ...
    fn expected_mysql_statement_structural_keyword_candidates(
        ...
        let expected_load_index_candidates =
            |items: &[String]| -> Option<&'static [&'static str]> {
        ...
        let expected_create_user_auth_candidates =
            |tail: &[String]| -> Option<&'static [&'static str]> {
                match tail {
                    ...
                    [a, b, _plugin, d] if a == "IDENTIFIED" && b == "WITH" && d == "AS" => {
                        None
                    }
                    [a, b, _plugin, d, e]
                        if a == "IDENTIFIED"
                    ...
                    [.., a, b] if a == "INITIAL" && b == "AUTHENTICATION" => {
                        Some(&["IDENTIFIED"])
                    }
                    [.., a, b, c]
        ...
        let expected_create_user_candidates =
            |items: &[String]| -> Option<&'static [&'static str]> {
                ...
                if items.get(idx).map(String::as_str) == Some("IF") {
                    ...
                    if items.get(idx).map(String::as_str) != Some("NOT") {
                        return None;
                    }
                    idx += 1;
                    if idx == items.len() {
        ...
        if matches!(
            ...
            return Some(MYSQL_EXPLAINABLE_STATEMENT_HEAD_KEYWORDS);
        }
    ...
    fn expected_referential_action_keyword_candidates(
        ...
        let references_idx = toks[..on].iter().enumerate().rev().find_map(|(idx, (_, token))| {
            matches!(token, SqlToken::Word(word) if word.eq_ignore_ascii_case("REFERENCES"))
                .then_some(idx)
        })?;
        let table_idx = toks[..references_idx]
            .iter()
            .enumerate()
    ...
    fn collect_expected_keyword_suggestions_with_expression_context(
        ...
        if let Some(candidates) = Self::expected_plsql_and_routine_keyword_candidates(
            tokens,
            context_end,
            db_type,
        )
        ...
        if let Some(candidates) = Self::expected_tool_command_supplement_keyword_candidates(
            tokens,
            context_end,
            db_type,
        )
        ...
        if let Some(candidates) =
            Self::expected_table_sample_keyword_candidates(tokens, context_end, deep_ctx.phase)
        {
            return Self::filter_expected_candidates(prefix, candidates);
        }
        if let Some(candidates) =
