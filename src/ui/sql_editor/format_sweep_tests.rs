...
const FORMAT_SWEEP_INLINE_QUERY_FRAME_REGRESSION: &str =
    "SELECT JSON_OBJECT ('flag' VALUE CASE WHEN active = 1 THEN 'Y' ELSE 'N' END, 'grand' VALUE (SELECT SUM(amount) FROM orders) RETURNING CLOB) AS payload FROM DUAL;";
const FORMAT_SWEEP_COMPACT_OPERATOR_REGRESSION: &str = "SELECT 1 FROM DUAL WHERE pos_id ^= 999;";
...
const FORMAT_SWEEP_QUERY_BRACKET_REGRESSION: &str =
    "SELECT JSON_SERIALIZE(JSON [SELECT JSON {'id':e.id} FROM event_log e ORDER BY e.id] RETURNING VARCHAR2(2000)), (SELECT COUNT(*) FROM event_log) FROM DUAL;";
...
fn is_direct_query_clause_word(word: &str) -> bool {
    matches!(
...
impl<'a> FormatSweepDocument<'a> {
    ...
    fn gap_mutation_policy(
        ...
        // A unit carrying foreign-language source is reproduced verbatim by the
        // formatter, so its own whitespace is payload, not layout.
        if self
            .statements
            .get(current.statement_index)
            .and_then(|statement| self.text.get(statement.start..statement.end))
            .is_some_and(|statement| {
                sql_text::carries_foreign_language_source(statement)
                    || sql_text::is_auto_terminated_tool_command(statement)
            })
        {
            return FormatSweepGapMutationPolicy::Preserve;
        }
...
fn format_sweep_audit_semantic_layout(
    ...
    for statement in &document.statements {
        ...
        {
            let mut record_split_phrase = |first_token_idx: usize,
                                           first: &FormatSweepScriptTokenSpan,
                                           last_token_idx: usize,
                                           last: &FormatSweepScriptTokenSpan,
                                           phrase: &str| {
                if !has_comment_between(first_token_idx, last_token_idx)
                    && line_index(first.start) != line_index(last.start)
                {
                    issues.push(FormatSweepIssue::new(
                        FormatSweepIssueKind::LineBreak,
                        formatted,
                        last.start,
                        format!("fixed SQL phrase `{phrase}` must remain on one line"),
                    ));
                }
            };

            for (pair_pos, pair) in meaningful.windows(2).enumerate() {
                let [(first_token_idx, first), (last_token_idx, last)] = pair else {
                    continue;
                };
                let followed_by_by = meaningful
                    .get(pair_pos.saturating_add(2))
                    .is_some_and(|(_, span)| word_is(span, "BY"));
                if word_is(first, "RESOURCE") && word_is(last, "GROUP") && !followed_by_by {
                    record_split_phrase(
                        *first_token_idx,
                        first,
                        *last_token_idx,
            ...
            let fixed_phrases: &[(&[&[&str]], &str)] = &[
                (&[&["IS"], &["A"], &["SET"]], "IS A SET"),
                (
                    &[&["NEXT", "PREVIOUS"], &["VALUE"], &["FOR"]],
                    "NEXT/PREVIOUS VALUE FOR",
                ),
                (
                    &[&["FOR"], &["PIVOT", "UNPIVOT"], &["IN"]],
                    "FOR PIVOT/UNPIVOT IN",
                ),
                (
            ...
            for (words, phrase) in fixed_phrases {
                for window in meaningful.windows(words.len()) {
                    if window
                        .iter()
                        .zip(words.iter())
                        .all(|((_, span), expected)| word_is_any(span, expected))
                    {
                        let (first_token_idx, first) = window[0];
                        let (last_token_idx, last) = window[window.len().saturating_sub(1)];
                        record_split_phrase(first_token_idx, first, last_token_idx, last, phrase);
                    }
                }
            }

            for from_pos in 1..meaningful.len().saturating_sub(1) {
                let (_, from) = meaningful[from_pos];
                if !word_is(from, "FROM")
                    || !word_is_any(meaningful[from_pos + 1].1, &["FIRST", "LAST"])
                    || !matches!(
                        &meaningful[from_pos - 1].1.token,
                ...
                let Some(open_pos) = open_pos.filter(|open_pos| *open_pos > 0) else {
                    continue;
                };
                if !word_is(meaningful[open_pos - 1].1, "NTH_VALUE") {
                    continue;
                }
                let (first_token_idx, first) = meaningful[from_pos - 1];
                let (last_token_idx, last) = meaningful[from_pos + 1];
                record_split_phrase(
                    first_token_idx,
                    first,
                    last_token_idx,
    ...
    for (header_idx, header_words) in words_by_line.iter().enumerate() {
        ...
        for clause_idx in body_idx.saturating_add(1)..=statement_end_idx {
            if first_word_delimiter_depth_by_line[clause_idx] != Some(query_delimiter_depth)
                || !words_by_line[clause_idx]
                    .first()
                    .is_some_and(|word| is_direct_query_clause_word(word))
            {
                continue;
            }
            checked = checked.saturating_add(1);
...
fn format_sweep_audit_query_bracket_depth(
    formatted: &str,
    db_type: DatabaseType,
) -> (usize, Vec<FormatSweepIssue>) {
    ...
    let line_starts_with_token = |span: &FormatSweepScriptTokenSpan| {
        let idx = line_index(span.start);
        formatted
            .get(line_starts.get(idx).copied().unwrap_or(0)..span.start)
            .is_some_and(|prefix| prefix.bytes().all(|byte| byte.is_ascii_whitespace()))
    };
    let mut delimiter_depths = Vec::with_capacity(document.tokens.len());
    let mut current_statement = None;
    let mut paren_depth = 0usize;
    let mut brace_depth = 0usize;
    let mut bracket_depth = 0usize;
    for span in &document.tokens {
        if current_statement != Some(span.statement_index) {
            current_statement = Some(span.statement_index);
            paren_depth = 0;
            brace_depth = 0;
            bracket_depth = 0;
        }
        delimiter_depths.push((paren_depth, brace_depth, bracket_depth));
        if let SqlToken::Symbol(symbol) = &span.token {
            match symbol.as_str() {
                "(" => paren_depth = paren_depth.saturating_add(1),
                ")" => paren_depth = paren_depth.saturating_sub(1),
                "{" => brace_depth = brace_depth.saturating_add(1),
    ...
    let mut checked = 0usize;
    let mut issues = Vec::new();
    for (open_idx, open) in document.tokens.iter().enumerate() {
        if !matches!(&open.token, SqlToken::Symbol(symbol) if symbol == "[") {
            continue;
        }
        let Some(body_idx) = (open_idx.saturating_add(1)..document.tokens.len()).find(|idx| {
            document.tokens[*idx].statement_index == open.statement_index
                && !matches!(document.tokens[*idx].token, SqlToken::Comment(_))
        }) else {
            continue;
        };
        let SqlToken::Word(body_word) = &document.tokens[body_idx].token else {
            continue;
        };
        if !crate::sql_text::is_subquery_head_keyword(body_word) {
            continue;
        }

        let mut depth = 0usize;
        let close_idx = (open_idx..document.tokens.len()).find(|idx| {
            let span = &document.tokens[*idx];
            if span.statement_index != open.statement_index {
                return false;
            }
            match &span.token {
                SqlToken::Symbol(symbol) if symbol == "[" => {
        ...
        let owner_line_idx = line_index(open.start);
        let body_line_idx = line_index(document.tokens[body_idx].start);
        let rendered_owner_indent = lines
            .get(owner_line_idx)
            .map_or(0, |line| leading_spaces(line));
        let enclosing_paren_indent = delimiter_depths[open_idx]
            .0
            .saturating_mul(FORMAT_SWEEP_INDENT_WIDTH);
        let owner_indent = rendered_owner_indent.max(enclosing_paren_indent);
        let expected_body_indent = owner_indent.saturating_add(FORMAT_SWEEP_INDENT_WIDTH);
        checked = checked.saturating_add(1);
        if body_line_idx == owner_line_idx
            || !line_starts_with_token(&document.tokens[body_idx])
            || lines
                .get(body_line_idx)
                .is_none_or(|line| leading_spaces(line) != expected_body_indent)
        {
            issues.push(FormatSweepIssue::new(
                FormatSweepIssueKind::FrameAlignment,
                formatted,
                document.tokens[body_idx].start,
                format!(
                    "JSON query-array body must start one frame below owner depth {owner_indent}"
                ),
            ));
        }

        let body_delimiter_depth = delimiter_depths[body_idx];
        let clause_start_idx = body_idx.saturating_add(1);
        for (span, &delimiter_depth) in document.tokens[clause_start_idx..close_idx]
            .iter()
            .zip(&delimiter_depths[clause_start_idx..close_idx])
        {
            let SqlToken::Word(word) = &span.token else {
                continue;
            };
            if delimiter_depth != body_delimiter_depth
                || !is_direct_query_clause_word(word)
                || !line_starts_with_token(span)
            {
                continue;
            }
            checked = checked.saturating_add(1);
            let clause_line_idx = line_index(span.start);
            if lines
                .get(clause_line_idx)
                .is_none_or(|line| leading_spaces(line) != expected_body_indent)
            {
                issues.push(FormatSweepIssue::new(
                    ...
                    span.start,
                    format!(
                        "JSON query-array direct clause must share body depth {expected_body_indent}"
                    ),
                ));
            }
        }
        ...
        checked = checked.saturating_add(1);
        let close = &document.tokens[close_idx];
        let close_line_idx = line_index(close.start);
        if close_line_idx == body_line_idx
            || !line_starts_with_token(close)
            || lines
                .get(close_line_idx)
                .is_none_or(|line| leading_spaces(line) != owner_indent)
        {
            issues.push(FormatSweepIssue::new(
                FormatSweepIssueKind::FrameAlignment,
                formatted,
                close.start,
                format!(
                    "multiline JSON query-array close must return to owner depth {owner_indent}"
                ),
            ));
        }
    }
...
const FORMAT_SWEEP_COMPACT_OPERATORS: &[&str] = &[
    "<=>", "<->", "<#>", "->>", "<<", ">>", "<=", ">=", "<>", "!=", "^=", "||", "**", ":=", "=>",
    "->",
];

fn format_sweep_compact_operator_fingerprint(
    text: &str,
    db_type: DatabaseType,
) -> Vec<(String, usize)> {
    let document = FormatSweepDocument::new(text, db_type);
    let mut operators = Vec::new();
    let mut idx = 0usize;

    while idx < document.tokens.len() {
        let first = &document.tokens[idx];
        if !matches!(first.token, SqlToken::Symbol(_)) {
        ...
        for end_idx in idx..document.tokens.len() {
            let span = &document.tokens[end_idx];
            if span.statement_index != first.statement_index
                || !matches!(span.token, SqlToken::Symbol(_))
                || (end_idx > idx && span.start != previous_end)
            {
                break;
            }
            let SqlToken::Symbol(symbol) = &span.token else {
                break;
            };
            ...
            if FORMAT_SWEEP_COMPACT_OPERATORS.contains(&candidate.as_str()) {
                longest_match = Some((candidate.clone(), end_idx));
            }
        }

        if let Some((operator, end_idx)) = longest_match {
            operators.push((operator, first.start));
...
fn format_sweep_audit_compact_operator_boundaries(
    source: &str,
    formatted: &str,
    db_type: DatabaseType,
) -> Option<FormatSweepIssue> {
    let source_operators = format_sweep_compact_operator_fingerprint(source, db_type);
    let formatted_operators = format_sweep_compact_operator_fingerprint(formatted, db_type);
    let source_symbols = source_operators
        .iter()
        .map(|(operator, _)| operator)
        .collect::<Vec<_>>();
    let formatted_symbols = formatted_operators
        .iter()
        .map(|(operator, _)| operator)
        .collect::<Vec<_>>();
    if source_symbols == formatted_symbols {
        return None;
    }

    let first_mismatch = source_symbols
        .iter()
        .zip(&formatted_symbols)
        .position(|(source, formatted)| source != formatted)
        .unwrap_or_else(|| source_symbols.len().min(formatted_symbols.len()));
    let marker_offset = formatted_operators
        .get(first_mismatch)
        .map_or(0, |(_, offset)| *offset);
    Some(FormatSweepIssue::new(
        FormatSweepIssueKind::ItemOrTokenChanged,
        formatted,
        marker_offset,
        format!(
            "formatting changed a compact SQL operator boundary at operator {}; source={source_symbols:?} formatted={formatted_symbols:?}",
            first_mismatch.saturating_add(1)
        ),
    ))
}
...
fn format_sweep_audit_identifier_case(
    ...
    if source_words.len() != formatted_word_indices.len() {
        let formatted_words = formatted_word_indices
            .iter()
            .filter_map(|idx| match &formatted_document.tokens.get(*idx)?.token {
                SqlToken::Word(word) => Some(word.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>();
        let first_mismatch = source_words
            .iter()
            .zip(&formatted_words)
            .position(|(source, formatted)| !source.eq_ignore_ascii_case(formatted))
            .unwrap_or_else(|| source_words.len().min(formatted_words.len()));
        let preview_start = first_mismatch.saturating_sub(3);
        let source_preview = source_words
            .iter()
            .skip(preview_start)
            .take(7)
            .copied()
            .collect::<Vec<_>>();
        let formatted_preview = formatted_words
            .iter()
            .skip(preview_start)
            .take(7)
            .copied()
            .collect::<Vec<_>>();
        let marker_offset = formatted_word_indices
            .get(first_mismatch)
            .and_then(|idx| formatted_document.tokens.get(*idx))
        ...
        return (
            ...
            vec![FormatSweepIssue::new(
                ...
                marker_offset,
                format!(
                    "identifier-case audit could not align source/formatted word tokens: {} -> {}; first mismatch at word {}; source={source_preview:?} formatted={formatted_preview:?}",
                    source_words.len(),
...
fn format_sweep_run_with_config(
    ...
    let (checked_query_bracket_depths, query_bracket_depth_issues) =
        format_sweep_audit_query_bracket_depth(&formatted, db_type);
    issues.extend(query_bracket_depth_issues);
    ...
    if let Some(issue) = format_sweep_audit_compact_operator_boundaries(source, &formatted, db_type)
    {
        issues.push(issue);
    }
    ...
    FormatSweepRun {
        ...
        checked_frame_depth_symmetries: frame_alignment_audit
            ...
            .saturating_add(checked_query_bracket_depths),
...
#[test]
fn formatting_sweep_preserves_multiline_quoted_identifier_payload() {
    let source = "SELECT 1 AS \"colümn\nbreak\", 2 FROM dual";
    let run = format_sweep_run(source, DatabaseType::Oracle);

    assert!(
        run.issues.is_empty(),
        "multiline quoted-identifier probe issues: {:#?}\n{}",
        run.issues,
        run.formatted
    );
    assert!(
        run.formatted.contains("\"colümn\nbreak\""),
        "quoted identifier payload must remain byte-for-byte intact:\n{}",
        run.formatted
    );
}
...
COMMIT WORK AND NO CHAIN NO RELEASE;"#;
    ...
    for db_type in [DatabaseType::MySQL, DatabaseType::MariaDB] {
        ...
        for transaction_statement in [
            "COMMIT WORK AND CHAIN NO RELEASE;",
            "ROLLBACK WORK AND CHAIN NO RELEASE;",
            "COMMIT WORK AND NO CHAIN NO RELEASE;",
        ] {
            assert!(
                run.formatted.contains(transaction_statement),
                "{db_type:?} transaction modifier was treated as a boolean condition: {transaction_statement}\n{}",
                run.formatted
            );
        }
    }
...
fn formatting_sweep_mysql_xa_and_show_server_statements_are_stable() {
    let source = r#"XA START 'sq-final-xa', 'branch-one', 1;
XA END 'sq-final-xa', 'branch-one', 1;
XA PREPARE 'sq-final-xa', 'branch-one', 1;
XA COMMIT 'sq-final-xa', 'branch-one', 1;
...
SHOW CREATE TABLE sq_manual_table;"#;
    ...
    for db_type in [DatabaseType::MySQL, DatabaseType::MariaDB] {
        ...
        assert!(
            run.formatted.contains("XA START 'sq-final-xa',")
                && run.formatted.contains("'branch-one',")
                && run.formatted.contains("1;"),
            "{db_type:?} XA three-part XID was not preserved:\n{}",
            run.formatted
        );
        assert!(
            run.managed_list_owner_kinds.contains(&ListOwnerKind::XaXid),
            "{db_type:?} XA XID components must have a typed list owner"
        );
        assert!(run
            .formatted
...
fn formatting_sweep_oracle_exec_bind_assignment_is_stable() {
    ...
    for source in [
        "EXEC OPEN :w9_cursor FOR SELECT bucket, qty FROM sq_bucket WHERE qty >= 40 ORDER BY bucket_id",
        "EXEC SELECT COUNT(*) INTO :w9_count FROM sq_bucket",
    ] {
        let run = format_sweep_run(source, DatabaseType::Oracle);
        assert!(
            run.issues.is_empty(),
            "Oracle line-terminated EXEC probe issues: {:#?}\n{}",
            run.issues,
            run.formatted
        );
        assert!(
            !run.formatted.contains('\n'),
            "SQL*Plus EXEC must remain one physical line:\n{}",
            run.formatted
        );
    }
}
...
#[test]
fn formatting_sweep_query_bracket_depth_audit_flags_missing_frame_depth() {
    let missing_depth = r#"CREATE JSON RELATIONAL DUALITY VIEW dv_demo AS
    ...
    let complete_depth = r#"CREATE JSON RELATIONAL DUALITY VIEW dv_demo AS
    SELECT JSON { '_id' :parent_owner.id, 'children' : [
        SELECT JSON { 'childId' :child_owner.id, 'amount' :child_owner.amount }
        FROM child_table child_owner WITH INSERT UPDATE DELETE
        WHERE child_owner.parent_id = parent_owner.id
    ...
    let (_, ddl_issues) =
        format_sweep_audit_ddl_query_body_depth(missing_depth, DatabaseType::Oracle);
    assert!(
        ddl_issues.is_empty(),
        "JSON object/array commas are not DDL query-list siblings: {ddl_issues:#?}"
    );

    let (checked, issues) =
        format_sweep_audit_query_bracket_depth(missing_depth, DatabaseType::Oracle);
    assert_eq!(checked, 4);
    ...
    let (checked, issues) =
        format_sweep_audit_query_bracket_depth(complete_depth, DatabaseType::Oracle);
    assert_eq!(checked, 4);
    assert!(issues.is_empty(), "{issues:#?}");
}

#[test]
fn formatting_sweep_query_bracket_inline_parent_frame_is_owned() {
    let mut config = AppConfig::new();
    config.sql_comma_list_layout = SqlCommaListLayout::Wrapped;
    let run = format_sweep_run_with_config(
        FORMAT_SWEEP_QUERY_BRACKET_REGRESSION,
        DatabaseType::Oracle,
        Some(&config),
    );
...
#[test]
fn formatting_sweep_semantic_layout_detects_split_fixed_phrases() {
    let oracle = r#"SELECT CASE WHEN values_tab(1) IS A
SET THEN 1 END FROM DUAL;
SELECT NTH_VALUE(v, 1)
FROM LAST OVER (ORDER BY id) FROM t;
SELECT * FROM t PIVOT (SUM(v) FOR
pivot IN (1));"#;
    let mysql = r#"CREATE RESOURCE
GROUP rg_demo TYPE = USER;
LOCK INSTANCE
FOR BACKUP;"#;
    let mariadb = r#"SELECT NEXT VALUE
FOR seq_demo;
FETCH
GROUP NEXT ROW;"#;

    let issues = format_sweep_audit_semantic_layout(oracle, DatabaseType::Oracle)
        .into_iter()
        .chain(format_sweep_audit_semantic_layout(
            mysql,
            DatabaseType::MySQL,
    ...
    for phrase in [
        ...
        assert!(
            messages.iter().any(|message| message.contains(phrase)),
            "split fixed phrase `{phrase}` was not detected: {messages:#?}"
        );
    }
}
...
#[test]
fn formatting_sweep_detects_split_compact_operator_boundary() {
    let source = "SELECT 1 FROM DUAL WHERE pos_id ^= 999;";
    let formatted_with_split = "SELECT 1 FROM DUAL WHERE pos_id ^ = 999;";
    let issue = format_sweep_audit_compact_operator_boundaries(
        source,
        formatted_with_split,
        DatabaseType::Oracle,
    )
    .expect("splitting a compact SQL operator should be reported");
...
fn formatting_sweep_mysql_top_level_current_diagnostics_keeps_item_list_attached() {
    ...
    let items =
        super::query_text::split_format_items_for_db_type(source, Some(DatabaseType::MySQL));
    let run = format_sweep_run(source, DatabaseType::MySQL);

    assert!(
        run.issues.is_empty(),
        "top-level GET CURRENT DIAGNOSTICS issues: {:#?}\n{}",
        run.issues,
        run.formatted
    );
    assert!(
        !run.formatted.contains("GET CURRENT DIAGNOSTICS;\n"),
        "the SQL*Plus GET command must not split a MySQL diagnostics statement:\nitems={items:#?}\n{}",
        run.formatted
    );
    assert!(
        run.formatted.contains("@condition_count = NUMBER")
            && run.formatted.contains("@affected_rows = ROW_COUNT;"),
        "diagnostics assignment items were detached from their statement:\n{}",
        run.formatted
    );
    assert!(
        run.managed_list_owner_kinds
            .contains(&ListOwnerKind::DiagnosticsItems),
        "top-level diagnostics statement did not create its typed list owner: {:?}\n{}",
        run.managed_list_owner_kinds,
        run.formatted
    );
}
...
fn formatting_sweep_additional_non_parenthesized_child_lists_have_typed_frames() {
    let cases: &[(DatabaseType, &str, &[ListOwnerKind])] = &[
        ...
        (
            DatabaseType::MariaDB,
            "UPDATE t1, t2 SET t1.v = 1, t2.v = 2; ALTER TABLE t1 ADD a INT, ADD b INT; CHECK TABLE t1, t2; CHECKSUM TABLE t1, t2; REPAIR TABLE t1, t2; CHECK VIEW v1, v2 FOR UPGRADE; REPAIR VIEW v1, v2 FROM MYSQL; CACHE INDEX t1 KEY (PRIMARY), t2 INDEX (PRIMARY) IN `default`; GRANT role_a, role_b TO user_a, user_b; CREATE PROCEDURE p() BEGIN DECLARE v_a, v_b INT; DO v_a, v_b; END;",
            &[
                ListOwnerKind::UpdateTargets,
                ListOwnerKind::AlterActions,
                ListOwnerKind::MaintenanceTables,
...
fn formatting_sweep_list_owner_inventory_covers_every_typed_variant() {
    let cases = [
        ...
        (
            DatabaseType::Oracle,
            "SELECT currency_code, amount FROM money ORDER BY currency_code, amount DESC FETCH FIRST 3 PARTITIONS BY currency_code, 2 ROWS ONLY;",
        ),
        (
            DatabaseType::MySQL,
        ...
        (
            DatabaseType::MySQL,
            "UPDATE t1, t2 SET t1.v = 1, t2.v = 2; ALTER TABLE t1 ADD a INT, ADD b INT; DROP TABLE IF EXISTS old_a, old_b; RENAME TABLE old_c TO new_c, old_d TO new_d; LOCK TABLES new_c READ, new_d WRITE; ANALYZE TABLE new_c, new_d; CREATE USER user_a IDENTIFIED BY 'x', user_b IDENTIFIED BY 'y'; XA START 'gtrid', 'branch', 1;",
        ),
    ];
...
#[test]
fn formatting_sweep_preserves_inline_mle_call_spec_terminator() {
    let source = "CREATE OR REPLACE FUNCTION reverse_text (word VARCHAR2) RETURN VARCHAR2\n\
...
/";
    ...
    assert!(
        run.formatted.contains("}};\n/"),
        "inline MLE call spec lost its mandatory semicolon:\n{}",
        run.formatted
    );
    assert!(run.issues.is_empty(), "{:#?}", run.issues);
}

#[test]
fn formatting_sweep_preserves_oracle_exponentiation_operator() {
    let source = "BEGIN\n  powered := base ** 10;\nEND;\n/";
    let run = format_sweep_run(source, DatabaseType::Oracle);

    assert!(
        run.formatted.contains("powered := base ** 10;"),
        "Oracle exponentiation was split:\n{}",
        run.formatted
    );
    assert!(run.issues.is_empty(), "{:#?}", run.issues);
}
...
fn formatting_sweep_all_files_generate_out_report_for_layout(layout: SqlCommaListLayout) -> usize {
    ...
    let built_in_regressions = FORMAT_SWEEP_FRAME_REGRESSION_CASES
        ...
        .chain([
            ...
            (
                DatabaseType::Oracle,
                FORMAT_SWEEP_COMPACT_OPERATOR_REGRESSION,
            ),
            (DatabaseType::Oracle, FORMAT_SWEEP_QUERY_BRACKET_REGRESSION),
...
#[test]
fn oracle_partitioned_fetch_comma_uses_a_structural_list_frame() {
    let source = "SELECT currency_code, amount FROM money ORDER BY currency_code, amount DESC FETCH FIRST 3 PARTITIONS BY currency_code, 2 ROWS ONLY;";
    let run = format_sweep_run(source, DatabaseType::Oracle);

    assert!(
        run.issues.is_empty(),
        "partitioned row limiting must own its comma-separated operands: {:#?}\n{}",
        run.issues,
        run.formatted
    );
    assert!(
        run.managed_list_owner_kinds
            .contains(&ListOwnerKind::PartitionedFetch),
        "partitioned row limiting did not create its typed list owner: {:?}\n{}",
        run.managed_list_owner_kinds,
        run.formatted
    );
}
