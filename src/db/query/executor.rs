...
impl QueryExecutor {
    ...
    fn is_word_char_at(chars: &[char], idx: usize) -> bool {
        chars
            .get(idx)
            .copied()
            .is_some_and(sql_text::is_identifier_char)
    ...
    /// True when `word` sits at `idx` as a whole word, ignoring case.
    fn word_matches_at(chars: &[char], idx: usize, word: &str) -> bool {
        let mut expected = word.chars();
        let mut offset = idx;
        for expected_char in expected.by_ref() {
            match chars.get(offset) {
                Some(actual) if actual.eq_ignore_ascii_case(&expected_char) => offset += 1,
                _ => return false,
            }
        }
        !Self::is_word_char_at(chars, offset)
    }
    ...
    pub(crate) fn extract_bind_names(sql: &str) -> Vec<String> {
        ...
        // An MLE unit carries foreign-language source: after the module DDL's
        // `AS`, and inside an inline `{{ ... }}` call-spec body, a `:` belongs to
        // JavaScript (`cond ? a : b`), never to a bind variable.
        let is_mle_module = sql_text::starts_create_mle_module(sql);
        let mut inline_foreign_brace_depth = 0usize;
        let mut saw_mle_word = false;
        ...
        while i < len {
            ...
            if inline_foreign_brace_depth > 0 {
                match c {
                    '{' => inline_foreign_brace_depth += 1,
                    '}' => inline_foreign_brace_depth -= 1,
                    _ => {}
                }
                i += 1;
            ...
            if c == '{' && next == Some('{') && saw_mle_word {
                inline_foreign_brace_depth = 2;
                i += 2;
                continue;
            }

            if matches!(c, 'm' | 'M' | 'a' | 'A')
                && !Self::is_word_char_at(&chars, i.wrapping_sub(1))
            {
                if Self::word_matches_at(&chars, i, "MLE") {
                    saw_mle_word = true;
                    last_token_was_string = false;
                    i += 3;
                    continue;
                }
                if is_mle_module && Self::word_matches_at(&chars, i, "AS") {
                    // The rest of the statement is the module's foreign source.
                    break;
                }
            }
    ...
    fn should_preserve_trailing_semicolon_for_execute(sql: &str) -> bool {
        ...
        // A stored unit owns its terminator: bodyless call specs
        // (`AS MLE MODULE m SIGNATURE 'f()';`, `AGGREGATE USING t;`,
        // `PIPELINED ... USING pkg;`) are stored incomplete without it and fail
        // to compile, and foreign source keeps whatever its last line ended with.
        if sql_text::starts_create_stored_unit(sql) {
            return true;
        }
    ...
    fn tool_command_lookalike_continues_open_sql(
        ...
        if sql_text::mysql_compatibility_for_sql(sql, preferred_db_type)
            && trimmed_line.starts_with('@')
        {
            let statement_tail_start = prefix.rfind(';').map_or(0, |idx| idx.saturating_add(1));
            if prefix
                .get(statement_tail_start..)
                .is_some_and(|tail| !Self::strip_comments(tail).trim().is_empty())
            {
                return true;
            }
        }
        ...
        (first_word.eq_ignore_ascii_case("SOURCE") && tail_upper.contains("CREATE PROPERTY GRAPH"))
            ...
            || (matches!(first_word.to_ascii_uppercase().as_str(), "CONN" | "CONNECT")
                && (tail_upper.contains("SELECT")
                    || (tail_upper.contains("CREATE PROFILE") && tail_upper.contains("LIMIT"))))
            ...
            || (matches!(
                first_word.to_ascii_uppercase().as_str(),
                "PASSW" | "PASSWORD"
            ) && (tail_upper.contains("ALTER USER") || tail_upper.contains("CREATE USER")))
    }
        ...
        F: FnMut((usize, usize), usize) -> bool,
    {
        ...
        while line_start < sql.len() {
            ...
            let is_mysql_diagnostics_item_line = !collector.current_is_empty()
                && collector
                    .builder
                    .starts_with_mysql_get_diagnostics_context();
            ...
            if collector.builder.is_idle()
                ...
                && !is_mysql_diagnostics_item_line
