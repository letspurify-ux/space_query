...
use crate::db::sql_classification::{strip_leading_comments_and_whitespace, SqlStatementAnalysis};
...
impl QueryExecutor {
    ...
    pub fn strip_leading_comments(sql: &str) -> String {
        strip_leading_comments_and_whitespace(sql).to_string()
    }
    ...
    pub fn leading_keyword(sql: &str) -> Option<String> {
        SqlStatementAnalysis::new(sql).leading_keyword_owned()
    }
    ...
    fn main_from_references_cte(
        ...
        let cte_names =
            Self::with_clause_cte_names(full_sql.get(..with_prefix_len).unwrap_or(full_sql));
        if cte_names.is_empty() {
            return false;
        }
        let Some(relation_name) = Self::first_from_relation_name(effective_sql, from_idx) else {
            return false;
        };
        cte_names
            .iter()
            .any(|cte_name| cte_name.eq_ignore_ascii_case(&relation_name))
    }
    ...
    fn with_clause_cte_names(with_prefix: &str) -> Vec<String> {
        ...
        for token in scanner {
            match token {
                ...
                ScanToken::Word { text, .. }
                    if saw_with && expect_name && text.eq_ignore_ascii_case("RECURSIVE") => {}
                ScanToken::Word { text, .. } if saw_with && expect_name => {
                    names.push(Self::normalize_relation_identifier(text));
                    expect_name = false;
                }
                ScanToken::Symbol { byte: b',', .. } if saw_with => {
                    expect_name = true;
                }
                _ => {}
            }
    ...
    fn first_from_relation_name(sql: &str, from_idx: usize) -> Option<String> {
        ...
        if table_ref.is_empty()
            ...
            || Self::starts_with_relation_invocation(table_ref)
        {
            return None;
        }

        let relation_len = table_ref
            .char_indices()
            .find_map(|(idx, ch)| {
        ...
        let relation = table_ref.get(..relation_len)?.trim();
        Some(Self::normalize_relation_identifier(relation))
    }

    fn normalize_relation_identifier(identifier: &str) -> String {
        identifier
            .rsplit('.')
            .next()
    ...
    fn single_table_rowid_expression(sql: &str, from_idx: usize) -> Option<String> {
        ...
        if Self::from_clause_relation_is_dual(table_ref) {
            return None;
        }
    ...
    fn from_clause_relation_is_dual(table_ref: &str) -> bool {
        ...
        let relation = table_ref
            ...
            .replace('"', "")
            .to_ascii_uppercase();
        relation == "DUAL" || relation.ends_with(".DUAL")
    }
