...
fn should_record_app_log(level: LogLevel, source: &str) -> bool {
    let is_oracle_thin_diagnostic = source == "oracle_thin" || source.starts_with("oracle_thin/");
    !is_oracle_thin_diagnostic || level == LogLevel::Error
}

pub fn log(level: LogLevel, source: &str, message: &str) {
    if !should_record_app_log(level, source) {
        return;
    }
...
mod logging_tests {
    ...
    #[test]
    fn oracle_thin_app_log_records_only_errors() {
        for level in [LogLevel::Debug, LogLevel::Info, LogLevel::Warning] {
            assert!(!should_record_app_log(level, "oracle_thin"));
            assert!(!should_record_app_log(level, "oracle_thin/connect"));
        }
        assert!(should_record_app_log(LogLevel::Error, "oracle_thin"));
        assert!(should_record_app_log(
            LogLevel::Error,
            "oracle_thin/connect"
        ));
        assert!(should_record_app_log(LogLevel::Info, "connection"));
    }
