...
pub(crate) const SQLPLUS_SET_OPTION_KEYWORDS: &[&str] = &[
    "APPINFO",
    "ARRAYSIZE",
    "AUTOCOMMIT",
    "AUTOPRINT",
...
/// True when `text` is `CREATE [OR REPLACE] <stored unit>` DDL - a PL/SQL unit,
/// a Java source or an MLE module. These are the only CREATE statements whose
/// own grammar can end in a semicolon (`END;`, a bodyless call spec such as
/// `AS MLE MODULE m SIGNATURE 'f()';` or `AGGREGATE USING t;`, or foreign source
/// whose last line ends in `;`), so the terminator must survive normalization
/// instead of being stripped like the one after `CREATE TABLE t (...)`.
pub(crate) fn starts_create_stored_unit(text: &str) -> bool {
    let mut words = text
    ...
    for word in words {
        match word.as_str() {
            "OR" | "REPLACE" | "EDITIONABLE" | "NONEDITIONABLE" | "FORCE" | "NOFORCE" => continue,
            "FUNCTION" | "PROCEDURE" | "PACKAGE" | "TYPE" | "TRIGGER" | "LIBRARY" | "JAVA"
            | "MLE" => return true,
            _ => return false,
        }
    }
...
/// True when `text` carries foreign-language source that a SQL formatter must
/// reproduce byte for byte: the body of a `CREATE MLE MODULE`, or the inline
/// `{{ ... }}` body of an MLE call spec. Reflowing either one corrupts the
/// routine, because `//`, backticks, `${}` and regular-expression literals mean
/// something entirely different to JavaScript than they do to SQL.
pub(crate) fn carries_foreign_language_source(text: &str) -> bool {
    if starts_create_mle_module(text) {
        return true;
    }

    // The inline form is rare, so the cheap substring test comes first.
    text.contains("{{")
        && text
            .split(|ch: char| !is_identifier_char(ch))
            .any(|word| word.eq_ignore_ascii_case("MLE"))
}

/// True when `text` starts the `CREATE [OR REPLACE] MLE MODULE` DDL. Everything
/// after that statement's `AS` is foreign-language source (JavaScript), not SQL,
/// so every SQL-level scanner - statement splitting, bind extraction,
/// highlighting - has to stop interpreting it.
pub(crate) fn starts_create_mle_module(text: &str) -> bool {
    let mut words = text
        .split(|ch: char| !is_identifier_char(ch))
        .filter(|word| !word.is_empty())
        .map(str::to_ascii_uppercase);
    ...
    // `MLE` was the object-type word; only the MODULE form carries a body.
    let mut after_mle = text
        .split(|ch: char| !is_identifier_char(ch))
        .filter(|word| !word.is_empty())
        .skip_while(|word| !word.eq_ignore_ascii_case("MLE"));
    after_mle.next();
    after_mle
        .next()
