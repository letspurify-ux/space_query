...
run_codex_fix() {
  ...
  local -a cmd_args=()
  prompt="oracle thin 개발에 누락된 부분이나 잘못 구현된 부분이 없는지 초정밀 리뷰 후 수정해줘. 회귀 없도록 테스트 케이스도 추가해줘. 마지막에 아래 각 항목별 결과 보고해줘. (1) python-oracledb의 oracle thin기준으로 상세 리뷰 (2) 데이터 송수신 및 메세지 구조 부분 (structure.md) (3) oracle db 버전별로 달라질 수 있는부분 (version_diff.md) (4) data type 부분 (data_type.md) (5) 예외 처리 부분 (7) windows, macos, linux 모두 정상 동작해야해"
