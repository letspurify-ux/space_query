...
pub struct OracleThinCancelHandle {
    ...
    force_stream: Option<Arc<TcpStream>>,
...
impl OracleThinCancelHandle {
    ...
    pub fn break_execution(&self) -> Result<(), OracleThinError> {
        ...
        let result = (|| {
            let stream = self.break_stream.as_ref().ok_or_else(|| {
                OracleThinError::new("Oracle thin cancel stream is not available")
            })?;
            let mut stream = stream
                .lock()
                .map_err(|_| OracleThinError::new("Oracle thin cancel stream lock poisoned"))?;
            let signal = send_break_signal(&mut stream, self.protocol_version, self.supports_oob)?;
            self.break_signal.store(signal.as_u8(), Ordering::SeqCst);
            Ok(())
        })();
        if result.is_err() {
    ...
    pub fn force_close(&self) -> Result<(), OracleThinError> {
        let previous_cancelled = self.cancelled.swap(true, Ordering::SeqCst);
        let result = (|| {
            let stream = self.force_stream.as_deref().ok_or_else(|| {
                OracleThinError::new("Oracle thin cancel stream is not available")
            })?;
        ...
        if result.is_err() {
            self.cancelled.store(previous_cancelled, Ordering::SeqCst);
        }
        result
    }
}
...
impl OracleThinSession {
    ...
    pub fn cancel_handle(&self) -> OracleThinCancelHandle {
        OracleThinCancelHandle {
            ...
            force_stream: self.stream.try_clone().ok().map(Arc::new),
...
mod tests {
    ...
    #[test]
    fn force_close_without_a_socket_reports_failure_and_allows_retry() {
        let cancelled = Arc::new(AtomicBool::new(false));
        let handle = super::OracleThinCancelHandle {
            cancelled: cancelled.clone(),
            break_signal: Arc::new(AtomicU8::new(BREAK_SIGNAL_NONE)),
            break_stream: None,
            force_stream: None,
        ...
        assert!(handle.break_execution().is_err());
        assert!(!cancelled.load(std::sync::atomic::Ordering::SeqCst));
        assert!(handle.force_close().is_err());
        assert!(!cancelled.load(std::sync::atomic::Ordering::SeqCst));
        ...
        assert!(
            ...
            "force-close failure must preserve an earlier graceful cancel"
        );
    }

    #[test]
    fn force_close_does_not_wait_for_the_graceful_break_stream_mutex() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let client = TcpStream::connect(listener.local_addr().unwrap()).unwrap();
        let (_server, _) = listener.accept().unwrap();
        let break_stream = Arc::new(Mutex::new(client.try_clone().unwrap()));
        let handle = super::OracleThinCancelHandle {
            cancelled: Arc::new(AtomicBool::new(false)),
            break_signal: Arc::new(AtomicU8::new(BREAK_SIGNAL_NONE)),
            break_stream: Some(break_stream.clone()),
            force_stream: Some(Arc::new(client.try_clone().unwrap())),
            protocol_version: 319,
            supports_oob: false,
        };
        let _held_break_stream = break_stream.lock().unwrap();
        let (sender, receiver) = std::sync::mpsc::channel();
        let retry_handle = handle.clone();

        std::thread::spawn(move || {
            let _ = sender.send(handle.force_close());
        });

        receiver
            .recv_timeout(Duration::from_millis(100))
            .expect("force close must not wait for the graceful break mutex")
            .expect("force close should shut down its independent socket clone");
        retry_handle
            .force_close()
            .expect("force close should be idempotent after the socket is already shut down");
    }
